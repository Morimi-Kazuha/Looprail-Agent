...
from benchmarks.looprailbench.reproduce import (
    ReproductionConfig,
    ReproductionReport,
    StageResult,
    render_report,
...
from benchmarks.looprailbench.scorecard import DimensionScore, ScorecardResult
...
def test_reproduction_report_renders_score_dimensions_and_artifact_path(tmp_path: Path) -> None:
    report = ReproductionReport(
        ...
        score=ScorecardResult(
            schema="looprail.bench.multidimensional-score.v1",
...
def test_reproduction_reuses_current_evidence_without_running_commands(tmp_path: Path) -> None:
    looprail_commit = subprocess.run(
        ("git", "rev-parse", "HEAD"),
        check=True,
        capture_output=True,
        text=True,
    ...
    _write_formal_summary(formal_summary, looprail_commit=looprail_commit)
    runtime_evidence = inputs / "runtime-evidence.json"
    runtime_payload = {
        "schema": "looprail.bench.runtime-scheduler-experiments.v1",
        "source_commit": looprail_commit,
        "claim_eligible": True,
    }
    ...
    tokenwise_payload = {
        "schema": "looprail.bench.tokenwise-cost.report.v1",
        "campaign": {"looprail_commit": looprail_commit},
    ...
    _write_formal_summary(formal_summary, looprail_commit="b" * 40)
...
def test_reproduction_runs_all_missing_stages_and_composes_score(tmp_path: Path) -> None:
    ...
    def execute(command: tuple[str, ...]) -> str:
        ...
        if "scheduler_experiments" in joined:
            ...
            path = output_root / looprail_commit / "plan" / "runs" / "scheduler-experiments.test.json"
...
def test_makefile_exposes_single_reproduction_command() -> None:
    ...
    assert "looprailbench-reproduce:" in makefile
    assert "python -m benchmarks.looprailbench.reproduce" in makefile
...
def _write_campaign_artifacts(
    ...
    *,
    ...
    outcome_root = output_root / "campaigns" / "suite" / looprail_commit / "ship"
...
def _write_formal_summary(
    ...
    *,
    ...
    spec = {
        "schema": "looprail.bench.experiment.v1",
        "evidence_schema": "looprail.bench.evidence.v1",
        "identity": {
