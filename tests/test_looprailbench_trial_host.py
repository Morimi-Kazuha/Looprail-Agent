...
from benchmarks.looprailbench.usage import RecordingProvider, UsageRecorder
...
from looprail.spine.events import Text, Usage
...
def test_trial_isolation_creates_non_overlapping_state_roots(tmp_path: Path) -> None:
    ...
    assert first.looprail_home != second.looprail_home
    assert first.everos_root != second.everos_root
    assert first.trace_root != second.trace_root
    assert first.workspace.is_dir()
    assert first.child_environment()["LOOPRAIL_HOME"] == str(first.looprail_home)
...
async def test_runtime_trial_host_builds_through_runtime_assembly(
    ...
    assembly = _Assembly()
    calls: list[tuple[Config, LooprailConfig]] = []

    def _assemble(config, looprail_config, **kwargs):
        calls.append((config, looprail_config))
        return assembly

    monkeypatch.setattr("looprail.cli._runtime_assembly.assemble_runtime", _assemble)
    ...
    assert calls == [(config, looprail_config)]
