...
from benchmarks.looprailbench.packs.tracing.overhead_experiment import (
...
async def test_tracing_campaign_runs_real_runtime_and_rebuilds_offline(tmp_path: Path) -> None:
    ...
    rebuilt = verify_campaign(output_root=tmp_path, expected_looprail_commit="a" * 40)
    ...
    with pytest.raises(TracingExperimentError, match="trace_receipts"):
        verify_campaign(output_root=tmp_path, expected_looprail_commit="a" * 40)
