"""Tests for ``load_looprail_config`` reading Looprail extension blocks
...
from looprail.config import looprail as ec_module
...
def stub_config_path(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("looprail.config.loader.get_config_path", _stub)
    monkeypatch.setattr("looprail.config.looprail.get_config_path", _stub)
    return p


def test_missing_config_falls_through_to_defaults(stub_config_path) -> None:

    cfg = ec_module.load_looprail_config()
...
def test_skill_forge_block_loaded_from_camel_case(stub_config_path: Path) -> None:
    """Match the format ``looprail onboard`` writes (camelCase via
...
def test_extension_keys_with_unknown_field_rejected(stub_config_path: Path) -> None:
    ...
    with pytest.raises(ValidationError, match="totally_made_up_field"):
        ec_module.load_looprail_config()
