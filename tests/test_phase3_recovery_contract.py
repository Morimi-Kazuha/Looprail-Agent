"""Phase 3 durable recovery, checkpoint and fresh-runtime contract tests."""
...
from pico.agent.effects import EffectClass, EffectJournal, EffectRecord, EffectStatus
from pico.agent.recovery import (
    ExecutionAction,
...
def test_recovery_trace_evidence_carries_session_task_effect_checkpoint_and_new_turn(tmp_path: Path) -> None:
    ...
    assert attributes["recovery.task"] == "pico.run.resume"
...
from pathlib import Path
from pico.agent.effects import EffectClass, EffectJournal, EffectRecord, EffectStatus
from pico.agent.recovery import RecoveryStateStore
from pico.session.manager import SessionManager
...
import sys
from pathlib import Path
from pico.agent.recovery import RecoveryProjector
