...
from benchmarks.looprailbench import rebuild_report, run
from benchmarks.looprailbench.claims import ClaimRuleResult, evaluate_claim_rules
from benchmarks.looprailbench.coverage import assess_pair_coverage
from benchmarks.looprailbench.protocol import (
    RetrievalContext,
    RetrievalExecution,
    TrialContext,
    TrialExecution,
...
from benchmarks.looprailbench.report import (
    FullReport,
...
from benchmarks.looprailbench.statistics import clustered_bootstrap_interval
        ...
        return PackDefinition(
            ...
            variants=(
                ...
                VariantSpec(
                    ...
                    settings={"history_manager": "looprail_curator"},
                ),
            ),
            pairs=(
                PairSpec(
...
async def test_full_report_rebuilds_pair_metrics_and_claims_from_artifacts(
    ...
    spec = ExperimentSpec(
        ...
        identity={"looprail_commit": "b" * 40, "model": "scripted/report"},
        claim_rules=(
            ClaimRule(
                rule_id="context-token-reduction",
                metric=metric,
...
async def test_report_rebuild_rejects_same_plan_trial_summary_tampering(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("context-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "1" * 40, "model": "scripted/report"},
    )
    registry = PackRegistry()
    registry.register(_TokenPack())
    ref = await run(spec, registry=registry)
...
async def test_report_rebuild_rejects_same_plan_pair_summary_tampering(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("context-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "2" * 40, "model": "scripted/report"},
    )
    registry = PackRegistry()
    registry.register(_TokenPack())
    ref = await run(spec, registry=registry)
...
async def test_report_rebuild_rejects_same_plan_retrieval_summary_tampering(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("retrieval-evidence",),
        output_root=tmp_path,
        identity={"looprail_commit": "3" * 40, "model": "scripted/report"},
    )
    registry = PackRegistry()
    registry.register(_RetrievalEvidencePack())
    ref = await run(spec, registry=registry)
...
async def test_selected_operational_failures_only_affect_pair_coverage(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("context-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "a" * 40, "model": "scripted/report"},
...
async def test_report_rebuild_rejects_missing_or_corrupt_retry_claim(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("context-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "4" * 40, "model": "scripted/report"},
...
async def test_report_rebuild_rejects_unexpected_over_quota_retry_claim(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("context-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "5" * 40, "model": "scripted/report"},
        execution=ExecutionPolicy(
            max_comparison_block_attempts=2,
            max_comparison_block_retries_total=1,
        ),
...
async def test_declared_pack_reducer_metrics_are_in_report(
    ...
    spec = ExperimentSpec(
        ...
        output_root=tmp_path,
        identity={"looprail_commit": "d" * 40, "model": "scripted/report"},
    )
    registry = PackRegistry()
...
async def test_context_v2_capability_validity_does_not_require_efficiency_pair_coverage(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("context-calibration",),
        output_root=tmp_path,
        identity={"looprail_commit": "e" * 40, "model": "scripted/report"},
    )
    registry = PackRegistry()
    registry.register(_DeclaredContextCapabilityReducerPack())
    monkeypatch.setattr(
        "benchmarks.looprailbench.packs.context.reduce_context_artifacts",
        lambda **_kwargs: {
...
async def test_declared_pack_ship_state_fails_closed_at_top_level(
    ...
    from benchmarks.looprailbench.packs.context import (
...
async def test_pack_reducer_collision_with_base_metric_fails_closed(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("context-calibration",),
        output_root=tmp_path,
        identity={"looprail_commit": "f" * 40, "model": "scripted/report"},
    )
    registry = PackRegistry()
    registry.register(_DeclaredContextReducerPack())
    ref = await run(spec, registry=registry)
...
async def test_invalid_measurement_never_exports_passing_metric(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("context-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "c" * 40, "model": "scripted/report"},
        claim_rules=(
            ClaimRule(
                rule_id="context-token-reduction",
                metric=metric,
