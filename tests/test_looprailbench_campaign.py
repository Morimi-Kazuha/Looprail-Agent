"""Contract tests for frozen historical LooprailBench campaign identities."""
...
import benchmarks.looprailbench.campaign as campaign
from benchmarks.looprailbench.__main__ import _CLI_MODES
...
from benchmarks.looprailbench.campaign import (
    CampaignError,
    CampaignMode,
    CampaignServices,
...
from benchmarks.looprailbench.canonical import canonical_digest
from benchmarks.looprailbench.claims import evaluate_claim_rules
...
_SUITE_PATH = (
    Path(__file__).resolve().parents[1] / "benchmarks" / "looprailbench" / "suites" / "agent_application_ship_1.yaml"
)
...
def _services(
    ...
    *,
    ...
    return CampaignServices(
        ...
        resolve_looprail_commit=resolve_commit,
...
async def test_smoke_is_credential_free_and_never_resolves_provider(
    ...
    assert not (product_home / "evidence" / "looprailbench" / "campaign-locks").exists()
...
async def test_paid_campaign_rejects_concurrent_writer_before_reservation_or_probe(
    ...
    def reserve_budget(
        ...
        del suite, mode, estimate, output_root, suite_digest, looprail_commit
        reservation_calls += 1
        return resolved
...
async def test_default_smoke_preserves_runtime_metrics(
    ...
    from benchmarks.looprailbench.campaign import _run_deterministic_gate

    monkeypatch.setattr(campaign, "_resolve_looprail_commit", lambda: "a" * 40)
...
async def test_runtime_evidence_rejects_git_drift_during_tracks(
    ...
    monkeypatch.setattr(campaign, "_resolve_looprail_commit", lambda: next(commits))
    monkeypatch.setattr(campaign, "_worktree_is_clean", lambda: True)
...
async def test_paid_campaign_rejects_environment_drift_before_execution(
    ...
    identities = iter(
        (
            {
                "schema": "looprail.bench.environment.v1",
                "dependency_lock": {"sha256": "a" * 64},
            },
            {
                "schema": "looprail.bench.environment.v1",
...
async def test_ship_runs_deterministic_then_one_preflight_then_calibration_and_formal(
    ...
    assert request.looprail_commit == "a" * 40
    ...
    assert all(spec.identity["looprail_commit"] == "a" * 40 for spec in calls.specs)
    ...
    assert outcome.preflight.looprail_commit == "a" * 40
...
async def test_default_ship_services_parse_config_and_assemble_real_pack_registry(
    ...
    monkeypatch.setattr(
        ("benchmarks.looprailbench.packs.memory_skill.semantic_effect._configured_embedding_environment"),
        lambda: (
            {},
...
async def test_ship_rejects_commit_drift_after_preflight(
    ...
    with pytest.raises(
        ...
        await run_campaign(
            ...
            services=replace(
                services,
                resolve_looprail_commit=lambda: next(commits),
            ),
        )
...
async def test_paid_campaign_rejects_unbound_commit_before_provider(
    ...
    with pytest.raises(CampaignError, match="full Git object id"):
        await run_campaign(
            ...
            services=replace(
                services,
                resolve_looprail_commit=lambda: "main",
            ),
        )
...
def test_makefile_exposes_checkout_only_smoke_and_ship_targets() -> None:
    ...
    assert "looprailbench-smoke:" in makefile
    assert "looprailbench:" in makefile
    assert ("uv run --frozen --all-extras --exact python -m benchmarks.looprailbench --mode smoke") in makefile
    assert ("uv run --frozen --all-extras --exact python -m benchmarks.looprailbench --mode ship") in makefile
