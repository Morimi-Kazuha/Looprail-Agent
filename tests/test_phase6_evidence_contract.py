"""Phase 6 evidence-chain contracts.

These tests use the existing Spine/AgentLoop/PicoBench paths.  The repository
...
from benchmarks.picobench.claims import ClaimState, evaluate_layered_claim, evaluate_paired_claim
...
from benchmarks.picobench.records import TrialStatus
from benchmarks.picobench.records import VerificationState
from benchmarks.picobench.registry import PackRegistry
from benchmarks.picobench.report import rebuild_full_report
from benchmarks.picobench.schema import ClaimRule, ExecutionPolicy, ExperimentSpec
from benchmarks.picobench.verifier import VerifierSeal, run_sealed_verifier
...
async def _run_repository_experiment(
    root: Path,
    *,
    pack: Phase6RepositoryPack | None = None,
    claim_rules: tuple[ClaimRule, ...] = (),
):
    selected_pack = pack or Phase6RepositoryPack()
    registry = PackRegistry()
    registry.register(selected_pack)
    ref = await run(
        _phase6_spec(root, claim_rules=claim_rules),
        registry=registry,
    )
...
def test_memory_trace_records_bounded_write_recall_and_turn_link(
    ...
    from pico.memory_engine import Memory, MemoryKind, MemoryScope, MemoryVerification
...
async def test_repository_pack_persists_task_trial_verifier_and_manifest_links(tmp_path: Path) -> None:
    ...
    for record in records.values():
        ...
        assert manifest["schema"] == "pico.picobench.reproducibility.v1"
...
class _InvalidMeasurementPack(Phase6RepositoryPack):
