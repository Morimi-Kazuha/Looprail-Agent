...
from looprail.spine.events import Text


class _ScriptedProvider:
    def get_default_model(self) -> str:
        return "scripted/looprailbench"

    async def chat_with_retry(self, **kwargs) -> LLMResponse:
        return LLMResponse(
            content="LOOPRAIL_BENCH_RUNTIME_OK",
            finish_reason="stop",
            usage={
                "prompt_tokens": 5,
                "completion_tokens": 2,
...
@pytest.mark.asyncio
async def test_looprailbench_host_runs_the_real_runtime_path(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    config = Config(
        agents={
            "defaults": {
                "workspace": str(workspace),
                "model": "scripted/looprailbench",
    ...
    assert [event.content for event in outlet.events if isinstance(event, Text)] == ["LOOPRAIL_BENCH_RUNTIME_OK"]
