...
from benchmarks.looprailbench.protocol import TrialContext
from benchmarks.looprailbench.records import TrialKey, TrialStatus
...
from looprail.config.looprail import ContextConfig, LooprailConfig
from looprail.config.schema import Config
from looprail.context_engine import TurnContext
...
class _ArtifactProvider(LLMProvider):
    ...
    async def chat(
        ...
        is_curator = any("You are Looprail Curator" in str(message.get("content", "")) for message in messages)
        if is_curator:
            return LLMResponse(
                content="no executable curator plan",
                usage={
...
async def test_runtime_context_runner_classifies_dispatched_cancelled_usage_for_retry(
    ...
    config.agents.defaults.model = provider.get_default_model()
    looprail_config = LooprailConfig(base=config)
    looprail_config.context.curator_timeout_seconds = 0.01
    pack = ContextPack(
        ContextTrack.CALIBRATION,
        runner=RuntimeContextTrialRunner(
            config=config,
