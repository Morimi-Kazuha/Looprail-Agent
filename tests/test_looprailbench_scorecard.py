...
from benchmarks.looprailbench.scorecard import (
    _formal_inputs,
    _runtime_inputs,
    _tokenwise_inputs,
    compute_scorecard,
...
def test_scorecard_reports_current_diagnostic_without_certifying_it() -> None:
    ...
    assert result.schema == "looprail.bench.multidimensional-score.v1"
...
def test_scorecard_accepts_current_negative_runtime_evidence() -> None:
    ...
    assert _runtime_inputs(evidence, looprail_commit=looprail_commit) == (True, False)


def test_scorecard_accepts_current_tokenwise_report() -> None:
    looprail_commit = "b" * 40
    payload = {
    ...
    assert _tokenwise_inputs(report, looprail_commit=looprail_commit) == (True, True)
...
def test_scorecard_requires_formal_manifest_from_current_commit(tmp_path) -> None:
    ...
    assert _formal_inputs(summary_path, looprail_commit=looprail_commit)["experiment_id"] == experiment_id

    with pytest.raises(ValueError, match="current Scorecard subject"):
        _formal_inputs(summary_path, looprail_commit="b" * 40)
