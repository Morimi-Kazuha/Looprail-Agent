"""Local SkillForge contract tests.

These tests pin the structural migration from ``agent/skills.py`` to
``skill_forge/{store,service,types}.py`` while preserving the exact external
behavior of the legacy ``SkillsLoader``. Keeping the test inputs local and
deterministic makes future refactors easy to audit.
...
from looprail.memory_engine.skill_forge.catalog import _escape_xml
from looprail.memory_engine.skill_local import SkillRegistry
from looprail.memory_engine.skill_local.registry import (
    _check_requirements,
...
def tmp_builtin(tmp_path: Path):
    ...
    _write_skill(
        ...
        frontmatter_extra='metadata: {"looprail":{"always":true}}',
    )
    ...
    _write_skill(
        ...
        frontmatter_extra='metadata: {"looprail":{"requires":{"bins":["this_bin_does_not_exist_xyz"]}}}',
    )
...
class TestFrontmatterParsing:
    ...
    def test_nested_metadata_looprail_wins(self):
        raw = '{"looprail": {"emoji": "🔧"}, "nanobot": {"emoji": "🤖"}}'
