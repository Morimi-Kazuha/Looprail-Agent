"""Phase 7 contracts for the reviewer-facing ``pico run`` surface.
...
from pico.agent.recovery.state import RecoveryStateStore
from pico.cli._run_surface import (
    CliRunReporter,
    RunEvidence,
    RunVerbosity,
...
from pico.spine import ChatType, Origin, Source, TurnOutcome, Usage
from pico.spine.events import ToolEvent, ToolPhase, TurnEnded, TurnFailed
...
def cli_fixture(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[Path, Path, Path]:
    ...
    source = Path(__file__).parent / "fixtures" / "phase7_demo"
    ...
    monkeypatch.setenv("PICO_TRACING_DIR", str(trace_root))
...
def test_phase7_verbosity_is_small_and_bounded() -> None:
...
def test_phase7_terminal_status_uses_lifecycle_not_text() -> None:
...
def test_phase7_reporter_renders_bounded_summary_from_durable_records(tmp_path: Path) -> None:
    lines: list[str] = []
    ...
    import pico.cli._run_surface as surface
...
def test_phase7_recovery_unknown_effect_is_visible_and_not_replayed() -> None:
    lines: list[str] = []
    state = RecoveryState(
...
def test_phase7_cli_happy_path_uses_real_runtime_and_trace(
    ...
    assert "PICO Run" in result.stdout
...
def test_phase7_cli_tool_validation_failure_is_explicit(
    cli_fixture: tuple[Path, Path, Path],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    workspace, config_path, trace_root = cli_fixture
...
def test_phase7_cli_failure_keeps_trace_and_nonzero_result(
    cli_fixture: tuple[Path, Path, Path],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr("pico.cli.agent_commands.make_provider", lambda _config: object())
    monkeypatch.setattr("pico.agent.loop.AgentLoop", _FailingLoop)
...
def test_phase7_cli_resume_is_a_fresh_turn_with_durable_read(
    cli_fixture: tuple[Path, Path, Path],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    workspace, config_path, _trace_root = cli_fixture
    ...
    class _ResumeLoop:
        ...
        async def run_turn(self, req, emit, _drain, **_kwargs):
            ...
            await emit(
                __import__("pico.spine.events", fromlist=["Text"]).Text(
    ...
    monkeypatch.setattr("pico.cli.agent_commands.make_provider", lambda _config: object())
    monkeypatch.setattr("pico.agent.loop.AgentLoop", _make_loop)
    result = runner.invoke(
        app,
        [
            "run",
...
def test_phase7_real_cli_boundary_creates_marker_then_resumes(
    cli_fixture: tuple[Path, Path, Path],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    workspace, config_path, _trace_root = cli_fixture
...
def test_phase7_quiet_output_is_redirect_safe(
    cli_fixture: tuple[Path, Path, Path],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    workspace, config_path, _trace_root = cli_fixture
    ...
    assert "Result" in result.stdout and "COMPLETED" in result.stdout
    assert "Evidence" in result.stdout
    assert "PICO Run" not in result.stdout
