...
from benchmarks.looprailbench.canonical import canonical_digest, canonical_json
from benchmarks.looprailbench.environment import (
...
@pytest.mark.skipif(
    os.name == "nt",
    reason=(
        "POSIX guest mount paths are currently validated with host Path semantics "
        "on Windows; cross-platform sandbox validation is outside R4."
    ),
)
