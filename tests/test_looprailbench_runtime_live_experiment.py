...
from benchmarks.looprailbench.packs.runtime.live_scheduler_experiment import (
    LiveSchedulerConfig,
    _analyze_live_pairs,
...
class _MarkerProvider(LLMProvider):
    ...
    async def chat(self, messages, **kwargs) -> LLMResponse:
        markers = re.findall(r"LOOPRAIL_LIVE_PERF_R\d{2}_(?:HOT|FG)_\d{2}", str(messages[-1]["content"]))
        marker = markers[-1]
...
async def test_live_arm_runs_real_agent_loop_with_marker_verification(tmp_path) -> None:
    ...
    payload = {
        "schema": "looprail.bench.runtime-live-scheduler.v2",
        "analysis": analysis,
        "repetitions": pairs,
