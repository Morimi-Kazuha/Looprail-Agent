"""Phase 4 budgeted Context Engine and long-turn acceptance contract tests."""
...
from pico.agent.recovery import RecoveryState
from pico.config.pico import ContextConfig
from pico.context_engine import (
    ContextAssembler,
    ContextBudgetError,
    HistoryTrimmer,
...
from pico.providers.base import GenerationSettings, LLMProvider, LLMResponse
...
def test_unknown_provider_limit_is_explicit_configured_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("pico.context_engine.budget.resolve_context_window", lambda *args, **kwargs: None)
...
def test_short_history_stays_below_budget_and_reports_keep_decision() -> None:
    ...
    trimmer = HistoryTrimmer(provider, "phase4-model", lambda: [], 2_000, runtime_margin_tokens=100)
...
def test_history_budget_drops_a_whole_low_priority_turn_and_keeps_tool_pair() -> None:
    ...
    trimmer = HistoryTrimmer(provider, "phase4-model", lambda: [], 550)
...
async def test_unknown_recovery_evidence_is_budgeted_and_not_persisted(tmp_path: Path) -> None:
    ...
    session_key = "cli:phase4-recovery"
    ...
    agent._resume_projections[session_key] = RecoveryState(
        ...
        projection_id="projection-phase4",
    )
    metadata: dict = {}
    budget_holder: dict = {}
