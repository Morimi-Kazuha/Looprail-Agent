...
from benchmarks.looprailbench.packs.context.runner import (
    _RoleScopedProvider,
    _trial_configs,
)
from benchmarks.looprailbench.packs.context.runner import (
...
from benchmarks.looprailbench.plan import compile_plan
...
from benchmarks.looprailbench.schema import ExperimentSpec
from looprail.config.looprail import LooprailConfig
from looprail.config.schema import Config
from looprail.context_engine.base import AssembledPrefix, AssemblyContext
...
from looprail.providers.base import ErrorClassification, GenerationSettings
...
def test_context_trial_binds_curator_to_frozen_campaign_model(
    ...
    looprail_config.context.curator_model = "gemini-2.5-flash"

    trial_config, trial_looprail = _trial_configs(
        config,
        looprail_config,
        workspace=tmp_path,
    ...
    assert trial_looprail.context.curator_model == "deepseek/deepseek-v4-flash"
    assert trial_config.agents.defaults.max_tokens == 500
    assert trial_looprail.context.protect_first_n == 1
...
def test_context_pack_freezes_benchmark_curator_steps_only() -> None:
    ...
    assert inspect.signature(CuratorSegmentBuilder.__init__).parameters["max_steps"].default == 12
