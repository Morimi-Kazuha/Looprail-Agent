...
def test_status_help_works() -> None:
    """``looprail status --help`` lists the command without error."""
    r = runner.invoke(app, ["status", "--help"])
    assert r.exit_code == 0
    assert "Show Looprail status" in r.stdout
...
def test_status_without_config_still_runs(tmp_config: Path) -> None:
    ...
    assert r.exit_code == 0
    assert "Looprail Status" in r.stdout
