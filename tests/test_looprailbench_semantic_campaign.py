...
from benchmarks.looprailbench.semantic_campaign import (
...
async def test_semantic_run_requires_exact_paid_approval(
    ...
    monkeypatch.setattr(
        "benchmarks.looprailbench.semantic_campaign._load_embedding_settings",
        _provider_settings,
    )
    monkeypatch.delenv(
        "LOOPRAIL_BENCH_SEMANTIC_PAID_APPROVAL",
        raising=False,
    )
    monkeypatch.setattr(
        "benchmarks.looprailbench.semantic_campaign._git_output",
...
async def test_complete_existing_root_rebuilds_before_paid_approval(
    ...
    _complete_artifact_root(
        ...
        looprail_commit=preflight.looprail_commit,
...
def test_semantic_rebuild_is_stable_and_keeps_claims_separate(
    ...
    manifest = {
        "schema": "looprail.bench.semantic-addendum.v1",
        "experiment_id": "p" * 64,
        "plan_digest": "p" * 64,
        "track": "calibration",
        "preflight": {
...
def test_semantic_v2_complete_artifact_rebuild_is_stable(
    ...
    assert first["schema"] == "looprail.bench.semantic-report.v2"
...
def _empty_record(
    ...
    *,
    ...
    record: dict[str, object] = {
        "schema": f"looprail.bench.semantic-retrieval-record.{version}",
        "plan_digest": "p" * 64,
        "key": {
...
def _complete_artifact_root(
    ...
    *,
    ...
    manifest = {
        "schema": f"looprail.bench.semantic-addendum.{version}",
