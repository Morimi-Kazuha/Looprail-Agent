...
from looprail.memory_engine import TokenBudget
from looprail.memory_engine.skill_forge import LocalSkillSource


def _portable_path_text(value: str) -> str:
    """Compare rendered paths independently of Windows separator escaping."""
...
class TestSkillForgeRouterAssembly:
    ...
    async def test_real_local_skill_injects_body_and_resolves_bundled_files(
        ...
        expected_run = _portable_path_text("Run " + str(skill_dir / "scripts" / "release.sh"))
        expected_checklist = _portable_path_text(
            f"[the checklist]({skill_dir / 'references' / 'CHECKLIST.md'})"
        )
        rendered = _portable_path_text(system)
        assert expected_run in rendered
        assert expected_checklist in rendered
