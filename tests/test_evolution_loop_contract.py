"""Contract tests for the existing Controlled Self-Evolution Loop.
...
from benchmarks.looprailbench.claims import ClaimState, evaluate_layered_claim
from benchmarks.looprailbench.fixtures.evidence_repository import EvidenceRepositoryPack
from benchmarks.looprailbench.records import VerificationState
from benchmarks.looprailbench.verifier import (
...
from looprail.evolver.candidate_manifest import ManifestGateError
from looprail.evolver.lineage import (
...
from looprail.evolver.orchestrator.config import Budget, OrchestratorConfig, Termination
from looprail.evolver.orchestrator.gates.paired import paired_lift
from looprail.evolver.orchestrator.gates.policy import CandidateOutcome, FrozenColdStartBaseline
...
from looprail.evolver.orchestrator.production import (
...
@pytest.fixture
def evolution_trace_root(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
...
def _subject_repo(tmp_path: Path, *, sealed_expected: str = "fixed") -> tuple[Path, str]:
    ...
    env = {
        **os.environ,
        "GIT_AUTHOR_NAME": "Looprail Phase8",
        "GIT_AUTHOR_EMAIL": "phase8@example.invalid",
        "GIT_COMMITTER_NAME": "Looprail Phase8",
...
def test_trace_lineage_requires_real_durable_failure(
    evolution_trace_root: Path,
    tmp_path: Path,
) -> None:
    evidence, trial_failure = _make_source_evidence(evolution_trace_root, tmp_path)
...
def test_trace_lineage_rejects_absent_or_successful_source(
    ...
    with pytest.raises(LineageError, match="missing|empty"):
        load_trace_failure_evidence(evolution_trace_root, "missing-run")
    ...
    with pytest.raises(LineageError, match="no failure"):
        load_trace_failure_evidence(evolution_trace_root, success_id)
...
def test_evolution_freeze_is_immutable_and_train_sealed_are_disjoint(
    ...
    source, _ = _make_source_evidence(evolution_trace_root, tmp_path)
...
def test_positive_trace_to_candidate_to_sealed_to_pending_human(
    ...
    result = _positive_run(tmp_path, evolution_trace_root)
...
def test_human_activation_actor_is_required_and_is_explicit(
    ...
    from looprail.evolver.activation import ActivationState, set_activation_state
    ...
    ready = set_activation_state(
        ...
        reason="Evolution fixture review",
    )
