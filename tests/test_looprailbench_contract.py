...
from benchmarks.looprailbench import ExperimentSpec, PackRegistry, rebuild_report, run
from benchmarks.looprailbench.artifacts import (
    ArtifactError,
    ArtifactStore,
)
from benchmarks.looprailbench.protocol import RetrievalContext, RetrievalExecution, TrialContext, TrialExecution
...
async def test_run_resume_and_rebuild_use_the_public_interface(tmp_path: Path) -> None:
    ...
    spec = ExperimentSpec(
        ...
        identity={"looprail_commit": "a" * 40, "model": "scripted/dummy"},
    )
...
async def test_retry_summaries_reference_every_immutable_attempt(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("dummy-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "b" * 40, "model": "scripted/retry"},
    )
...
async def test_measurable_task_failure_does_not_consume_block_retry(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("dummy-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "e" * 40, "model": "scripted/failure"},
...
async def test_experiment_retry_quota_is_shared_across_comparison_blocks(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("dummy-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "f" * 40, "model": "scripted/retry"},
        execution=ExecutionPolicy(
            max_comparison_block_attempts=2,
            max_comparison_block_retries_total=1,
        ),
...
async def test_partial_retry_attempt_is_not_persisted_or_spliced_on_resume(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("dummy-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "1" * 40, "model": "scripted/retry"},
        execution=ExecutionPolicy(
            max_comparison_block_attempts=2,
            max_comparison_block_retries_total=1,
        ),
...
async def test_run_rejects_a_concurrent_writer_for_the_same_experiment(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("dummy-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "2" * 40, "model": "scripted/lock"},
    )
...
async def test_resume_rebuilds_corrupt_retrieval_case_from_selected_attempt(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("dummy-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "c" * 40, "model": "scripted/retry"},
    )
...
async def test_resume_rebuilds_corrupt_retrieval_block_attempt_refs(
    ...
    spec = ExperimentSpec(
        ...
        pack_ids=("dummy-v1",),
        output_root=tmp_path,
        identity={"looprail_commit": "d" * 40, "model": "scripted/retry"},
    )
