"""Focused contract tests for the shipping ToolRegistry boundary.

These tests deliberately exercise the existing Registry rather than introducing a
parallel runtime abstraction. They record the supported contract at the
...
from looprail.agent.effects import EffectClass, EffectJournal, EffectStatus
from looprail.agent.tools.base import Tool, ToolResult
from looprail.agent.tools.execution import ToolCapability, ToolEffect, ToolExecutionContext
from looprail.agent.tools.filesystem import ReadFileTool, WriteFileTool
from looprail.agent.tools.registry import ToolRegistry
from looprail.agent.tools.shell import ExecTool
from looprail.sandbox import ExecResult, SandboxExecutor


def _context() -> ToolExecutionContext:
    return ToolExecutionContext(
        call_id="call-tool-runtime",
        session_key="session-tool-runtime",
        turn_id="turn-tool-runtime",
    )
...
async def test_successful_read_has_committed_read_effect(tmp_path: Path) -> None:
    ...
    assert record.session_key == "session-tool-runtime"
    assert record.turn_id == "turn-tool-runtime"
