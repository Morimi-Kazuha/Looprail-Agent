...
from benchmarks.looprailbench import budget as looprailbench_budget
from benchmarks.looprailbench.budget import (
    BudgetGuardedProvider,
    ProviderBudgetConfig,
    ProviderBudgetLedger,
)
from benchmarks.looprailbench.canonical import to_primitive
from benchmarks.looprailbench.packs.memory_skill import (
...
from benchmarks.looprailbench.packs.memory_skill import e2e as memory_skill_e2e
from benchmarks.looprailbench.packs.memory_skill import runtime as memory_skill_runtime
from benchmarks.looprailbench.packs.memory_skill import worker as memory_skill_worker
from benchmarks.looprailbench.packs.memory_skill.models import CrossSessionTask
from benchmarks.looprailbench.packs.memory_skill.retrieval import _FixtureMemoryBackend
from benchmarks.looprailbench.packs.memory_skill.runtime import (
    ScriptedCrossSessionProvider,
    _freeze_looprail_config,
    _freeze_runtime_config,
)
from benchmarks.looprailbench.packs.memory_skill.semantic_fixtures import (
    semantic_fixture,
)
from benchmarks.looprailbench.protocol import RetrievalContext, TrialContext
...
from looprail.context_engine.segments import MemorySegmentBuilder
...
def test_memory_skill_tasks_are_loaded_from_frozen_manifests() -> None:
    ...
    manifest_path = Path(__file__).parents[1] / "benchmarks" / "looprailbench" / "tasks" / "memory_skill" / "formal.json"
    payload = json.loads(manifest_path.read_text(encoding="utf-8"))

    assert payload["schema"] == "looprail.bench.memory-skill-tasks.v1"
...
def test_real_runner_freezes_model_budget_and_routing(
    ...
    looprail_config = LooprailConfig(base=frozen)
    looprail_config.token_wise.smart_routing.enabled = True
    frozen_looprail = _freeze_looprail_config(
        looprail_config,
        frozen,
        task,
    )
    assert frozen_looprail.token_wise.smart_routing.enabled is False
...
async def test_real_runner_uses_frozen_pack_budget_and_four_call_stage_split(
    ...
    experiment = ExperimentSpec(
        ...
        identity={"looprail_commit": "1" * 40, "model": "provider/exact-model"},
...
async def test_real_provider_worker_bootstrap_uses_stage_budget_scopes(
        ...
        json.dumps(
            {
                ...
                "looprail_config": looprail_config.model_dump(mode="json"),
            },
            sort_keys=True,
        ),
        encoding="utf-8",
    ...
    scopes = []
    actual_budget_scope = looprailbench_budget.provider_call_budget_scope
...
async def test_retrieval_records_are_anonymous_and_keep_memory_and_skill_separate(
    ...
    experiment = ExperimentSpec(
        ...
        pack_ids=(definition.pack_id,),
        output_root=tmp_path,
        identity={"looprail_commit": "a" * 40, "model": "scripted/deterministic"},
    )
    memory_suite, skill_suite = definition.retrieval_suites
...
async def test_retrieval_adapter_executes_product_segments_and_router(
    ...
    experiment = ExperimentSpec(
        ...
        pack_ids=(definition.pack_id,),
        output_root=tmp_path,
        identity={"looprail_commit": "c" * 40, "model": "scripted/deterministic"},
    )
...
async def test_formal_retrieval_micro_suite_has_separate_claim_gates(
    ...
    experiment = ExperimentSpec(
        ...
        output_root=tmp_path,
        identity={"looprail_commit": "b" * 40, "model": "scripted/deterministic"},
    )
...
async def test_artifact_reducer_separates_contract_and_real_claims(
    ...
    experiment = ExperimentSpec(
        ...
        pack_ids=(definition.pack_id,),
        output_root=tmp_path,
        identity={"looprail_commit": "f" * 40, "model": "scripted/deterministic"},
    )
    measurements = await run_retrieval_micro_suite(pack, experiment)
    retrieval_records = [
        {
