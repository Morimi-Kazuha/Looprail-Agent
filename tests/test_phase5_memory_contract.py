"""Phase 5 contract tests for the existing structured Memory authority.
...
from pico.config.pico import ContextConfig, MemoryConfig, PicoConfig
from pico.config.schema import Config
from pico.context_engine import AssemblyContext, ContextAssembler, TurnContext, build_context_engine
...
def test_A_write_policy_provenance_and_item_schema_are_explicit(tmp_path: Path) -> None:
    ...
    assert record["schema"] == "pico.memory.item.v1"
...
def test_optional_backend_unavailable_fails_closed(tmp_path: Path) -> None:
    config = PicoConfig(memory=MemoryConfig(backend="missing-optional-memory"))

    with pytest.raises(PluginNotFoundError):
        maybe_build_memory_backend(tmp_path, config, registry=PluginRegistry())


def test_J_actual_pico_run_reaches_structured_memory_segment(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Enter through the same Typer command used by the approved ``pico run`` surface."""
