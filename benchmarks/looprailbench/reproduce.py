...
from looprail.utils.portable_lock import LockTimeoutError, file_lock
...
def _run_reproduction_locked(
    ...
    *,
    ...
    try:
        ...
        runtime_current, runtime_claim = _runtime_inputs(runtime_record, looprail_commit=looprail_commit)
...
def _validate_reused_inputs(
    *,
    ...
    if formal_summary is not None:
        _validate_formal_summary(formal_summary, looprail_commit=looprail_commit)
    if runtime_evidence is not None:
        _runtime_inputs(_read_json(runtime_evidence), looprail_commit=looprail_commit)
    if tokenwise_report is not None:
        _tokenwise_inputs(_read_json(tokenwise_report), looprail_commit=looprail_commit)
...
def _validate_environment(config: ReproductionConfig) -> None:
    ...
    if git is None:
        raise ValueError("git is required to validate the LooprailBench checkout")
    ...
    if completed.stdout.strip():
        raise ValueError("LooprailBench reproduction requires a clean worktree")
    looprail_commit = _current_looprail_commit()
    ...
    if config.tokenwise_report is None and retained_tokenwise.is_file():
        _tokenwise_inputs(_read_json(retained_tokenwise), looprail_commit=looprail_commit)
    ...
    if config.formal_summary is None and isinstance(retained_scorecard, Path):
        _validate_formal_summary(retained_scorecard, looprail_commit=looprail_commit)
...
def _runtime_stage(
    ...
    *,
    ...
    existing = _find_runtime_evidence(root, looprail_commit=looprail_commit)
    ...
    _run_command(
        ...
        (
            sys.executable,
            "-m",
            "benchmarks.looprailbench.packs.runtime.scheduler_experiments",
            "--output-root",
            str(root),
        ),
        execute_command=execute_command,
    ...
    evidence = _find_runtime_evidence(root, looprail_commit=looprail_commit)
    if evidence is None:
...
def _tokenwise_stage(
    ...
    *,
    ...
    if report.is_file():
        _tokenwise_inputs(_read_json(report), looprail_commit=looprail_commit)
    ...
    for mode in ("preflight", "formal"):
        _run_command(
            ...
            (
                sys.executable,
                "-m",
                "benchmarks.looprailbench.tokenwise_cost_campaign",
                "--mode",
                mode,
                "--output-root",
                str(root),
...
def _scorecard_stage(
    ...
    *,
    ...
    if reused is not None:
        _validate_formal_summary(reused, looprail_commit=looprail_commit)
        return reused, "reused"
    existing = _campaign_artifacts(root, looprail_commit=looprail_commit)
    if existing is not None:
        _validate_formal_summary(existing, looprail_commit=looprail_commit)
        return existing, "resumed"
    _run_command(
        "context-tool",
        (
            sys.executable,
            "-m",
            "benchmarks.looprailbench.scorecard_campaign",
            "ship",
            "--output-root",
            str(root),
            "--runtime-evidence",
    ...
    summary = _campaign_artifacts(root, looprail_commit=looprail_commit)
    if summary is None:
        raise ReproductionError("Context and Tool/MCP stage produced no formal summary")
    _validate_formal_summary(summary, looprail_commit=looprail_commit)
...
def _find_runtime_evidence(root: Path, *, looprail_commit: str) -> Path | None:
    candidates = sorted(root.glob("**/scheduler-experiments.*.json"))
    valid = []
    for path in candidates:
        try:
            _runtime_inputs(_read_json(path), looprail_commit=looprail_commit)
...
def _campaign_artifacts(
    ...
    *,
    ...
    outcomes = sorted(root.glob(f"campaigns/*/{looprail_commit}/ship/campaign-outcome.json"))
    if not outcomes:
        return None
    if len(outcomes) > 1:
...
def _validate_formal_summary(path: Path, *, looprail_commit: str) -> None:
    _formal_inputs(path, looprail_commit=looprail_commit)
...
def _freeze_manifest(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        "schema": "looprail.bench.reproduction-manifest.v1",
        "looprail_commit": looprail_commit,
...
def _current_looprail_commit() -> str:
    git = shutil.which("git")
    if git is None:
...
def _write_report(report: ReproductionReport) -> None:
    payload: dict[str, Any] = {
        "schema": "looprail.bench.reproduction-report.v1",
        "status": report.status,
        "looprail_commit": report.looprail_commit,
        "error": report.error,
        "stages": [
            {
                "name": stage.name,
    ...
    rows = [
        "# LooprailBench reproduction report",
        "",
        f"- Status: {report.status}",
        f"- Looprail commit: `{report.looprail_commit}`",
...
def render_report(
    ...
    *,
    ...
    if score is not None:
        ...
        target.print(
            f"LooprailBench score: {score.diagnostic_score:.2f} / 100 (diagnostic, {certification})",
        )
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run or reuse every LooprailBench Scorecard track and render one report.",
    )
    parser.add_argument("--output-root", type=Path)
    parser.add_argument(
        "--execute-paid-campaign",
        action="store_true",
        default=os.environ.get("LOOPRAIL_BENCH_EXECUTE_PAID") == "1",
    )
...
def main() -> int:
    ...
    looprail_commit = _current_looprail_commit()
    output_root = args.output_root or _environment_path("LOOPRAIL_BENCH_REPRODUCTION_ROOT")
    if output_root is None:
        output_root = (
            Path(__file__).resolve().parents[2] / ".looprail" / "evidence" / "looprailbench-reproduction" / looprail_commit
        )
    report = run_reproduction(
        ReproductionConfig(
            output_root=output_root,
            execute_paid_campaign=args.execute_paid_campaign,
            formal_summary=(args.formal_summary or _environment_path("LOOPRAIL_SCORECARD_FORMAL_SUMMARY")),
            runtime_evidence=(args.runtime_evidence or _environment_path("LOOPRAIL_SCORECARD_RUNTIME_EVIDENCE")),
            tokenwise_report=(args.tokenwise_report or _environment_path("LOOPRAIL_SCORECARD_TOKENWISE_REPORT")),
        ),
