...
from benchmarks.looprailbench.packs.tokenwise_cost.live import CampaignConfig, CampaignError, load_task_corpus
from benchmarks.looprailbench.packs.tokenwise_cost.runner import (
    load_deepseek_key,
    run_cache_preflight,
    run_formal_campaign,
...
_CORPUS_PATH = _REPOSITORY_ROOT / "benchmarks" / "looprailbench" / "tasks" / "tokenwise_cost" / "formal.json"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=_REPOSITORY_ROOT / ".looprail" / "evidence" / "call-efficiency-cost-current",
    )
...
async def _run(mode: str, output_root: Path) -> dict:
    ...
    if mode == "plan":
        return {
            "schema": "looprail.bench.call-efficiency-cost.plan.v1",
            "looprail_commit": commit,
            "model": config.model,
    ...
    if mode == "verify":
        return verify_retained_campaign(output_root=output_root, corpus=corpus, looprail_commit=commit)
...
def _current_commit() -> str:
    ...
    if git is None:
        raise CampaignError("git is required to freeze the Looprail commit")
