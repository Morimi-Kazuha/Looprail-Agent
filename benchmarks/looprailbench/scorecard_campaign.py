...
_RUNTIME_SUBJECT_PATHS = (
    ...
    "pyproject.toml",
    "uv.lock",
    "benchmarks/looprailbench/packs/runtime",
)
...
def reused_runtime_gate(
    ...
    *,
    ...
    evidence_commit = str(evidence.get("looprail_commit", ""))
    if (
...
async def run_scorecard_campaign(
    ...
    *,
    ...
    looprail_commit = defaults.resolve_looprail_commit()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the current LooprailBench Scorecard campaign.",
    )
    parser.add_argument(
        "action",
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path(".looprail") / "evidence" / "looprailbench-scorecard-v1",
    )
    ...
    try:
        ...
        parser.exit(2, f"LooprailBench Scorecard aborted: {exc}\n")
    print(
        json.dumps(
