"""Deterministic repository-style fixture used by evidence-chain tests.

This fixture is intentionally a tiny local Python repository. It exercises the
existing LooprailBench Task/Trial/Verifier/Pair path without a model, a network
...
from benchmarks.looprailbench.reproducibility import capture_reproducibility_manifest
from benchmarks.looprailbench.schema import PackDefinition, PairSpec, TaskSpec, VariantSpec
from benchmarks.looprailbench.verifier import RepositoryTaskVerifier, VerifierSeal, run_sealed_verifier


@dataclass(frozen=True)
class EvidenceRepositoryFixture:
    # These phase6-prefixed values are serialized evidence identities from the
    # accepted benchmark contract; the public module/class names are descriptive.
...
class EvidenceRepositoryPack:
    """A small Pack adapter that reuses the ordinary LooprailBench harness."""

    fixture = EvidenceRepositoryFixture()
...
__all__ = ["EvidenceRepositoryFixture", "EvidenceRepositoryPack"]
