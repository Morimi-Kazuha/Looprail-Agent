...
from benchmarks.looprailbench.canonical import (
    canonical_bytes,
    canonical_digest,
)
from benchmarks.looprailbench.packs.memory_skill.fixtures import (
    anonymous_item_id,
)
...
from benchmarks.looprailbench.packs.memory_skill.semantic_runtime import (
...
SEMANTIC_SCHEMA = "looprail.bench.semantic-addendum.v1"
SEMANTIC_SCHEMA_V2 = "looprail.bench.semantic-addendum.v2"
...
DEFAULT_OUTPUT_ROOT = Path(".looprail/evidence/looprailbench-semantic")
DEFAULT_OUTPUT_ROOT_V2 = Path(".looprail/evidence/looprailbench-semantic-v2")
APPROVAL_ENV = "LOOPRAIL_BENCH_SEMANTIC_PAID_APPROVAL"
APPROVAL_ENV_V2 = "LOOPRAIL_BENCH_SEMANTIC_V2_PAID_APPROVAL"
    ...
    *,
    ...
    if experiment_root.exists():
        ...
        if (
            ...
            or existing.get("looprail_commit") != preflight.looprail_commit
    ...
    fixture = semantic_fixture(track, version=version)
    with tempfile.TemporaryDirectory(prefix=f"looprail-semantic-{track}-") as directory:
...
def rebuild_semantic_report(experiment_root: Path) -> dict[str, Any]:
    ...
    if not plan_digest or any(
        record.get("schema") != f"looprail.bench.semantic-retrieval-record.{version}"
    ...
    payload = {
        "schema": f"looprail.bench.semantic-report.{version}",
        ...
        "looprail_commit": preflight.get("looprail_commit"),
    ...
    cv_payload = {
        "schema": f"looprail.bench.semantic-cv-metrics.{version}",
        "experiment_id": manifest["experiment_id"],
...
def _load_embedding_settings() -> Any:
    ...
    from looprail.config.update_everos import configure_everos_env
...
def _semantic_budget_ledger_path(
    ...
    *,
    ...
    root = (
        budget_root if budget_root is not None else get_product_home() / "evidence" / "looprailbench" / "semantic-approvals"
    )
...
def _inspect_semantic_budget_ledger(
    ...
    *,
    ...
    expected_high_water = {
        "schema": "looprail.bench.provider-budget-high-water.v1",
...
def _validate_calibration_report(
    ...
    *,
    ...
    if (
        ...
        or report.get("looprail_commit") != looprail_commit
...
def _semantic_record_seal(
    ...
    *,
    ...
    return {
        "schema": (f"looprail.bench.semantic-record-seal.{_semantic_version(semantic_schema)}"),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the checkout-only LooprailBench semantic addendum.",
    )
    parser.add_argument(
        "--mode",
    ...
    try:
        ...
        parser.exit(2, f"LooprailBench semantic addendum aborted: {exc}\n")
