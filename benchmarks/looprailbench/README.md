# LooprailBench Ship-1

LooprailBench is Looprail's checkout-only evaluation campaign. It runs the real Looprail
Runtime through frozen, single-axis comparisons and rebuilds every result from
terminal artifacts. It is not part of the `looprail` wheel and does not add a
public `looprail bench` command.
...
The paid command uses the normal Looprail configuration, but only proceeds when it
...
```bash
LOOPRAIL_BENCH_EXECUTE_PAID=1 make looprailbench-reproduce
```
...
`.looprail/evidence/looprailbench-reproduction/<looprail-commit>/`. Repeating the command
...
```bash
LOOPRAIL_SCORECARD_FORMAL_SUMMARY=/absolute/path/to/summary.json \
LOOPRAIL_SCORECARD_RUNTIME_EVIDENCE=/absolute/path/to/runtime-evidence.json \
LOOPRAIL_SCORECARD_TOKENWISE_REPORT=/absolute/path/to/tokenwise-report.json \
  make looprailbench-reproduce
```
...
```bash
make looprailbench-scorecard-estimate
```

Run the paid campaign with:

```bash
make looprailbench-scorecard-ship
```

Runtime evidence is not required to measure the independent Context and
Tool/MCP Packs. When `LOOPRAIL_SCORECARD_RUNTIME_EVIDENCE` is supplied, the
campaign verifies that its Looprail product code, dependency identity, and Runtime
...
LOOPRAIL_SCORECARD_RUNTIME_EVIDENCE=/absolute/path/to/runtime-evidence.json \
LOOPRAIL_SCORECARD_TOKENWISE_REPORT=/absolute/path/to/tokenwise-report.json \
LOOPRAIL_SCORECARD_MEMORY_SUMMARY=/absolute/path/to/memory-summary.json \
LOOPRAIL_SCORECARD_MEMORY_HANDOFF=/absolute/path/to/memory-handoff.json \
LOOPRAIL_SCORECARD_PREREGISTERED=1 \
  make looprailbench-scorecard-score
```

Runtime, Memory, and preregistration inputs are optional. The Memory summary
is accepted only with a digest-bound handoff that names the current Looprail
...
The suite at
`benchmarks/looprailbench/suites/agent_application_ship_1.yaml` freezes:
...
Before any paid call, LooprailBench includes existing ledger spend and two live
...
`.looprail/evidence/looprailbench/` and are not committed.
