...
def _formal_inputs(
    ...
    *,
    ...
    if (
        ...
        or identity.get("looprail_commit") != looprail_commit
...
def _current_looprail_commit() -> str:
    ...
    if git is None:
        raise RuntimeError("git is required to resolve the Looprail commit")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compute the LooprailBench multidimensional diagnostic score.",
    )
    ...
    current_commit = _current_looprail_commit()
    formal_summary = _formal_inputs(
        args.formal_summary,
        looprail_commit=current_commit,
    )
...
def _runtime_inputs(
    ...
    *,
    ...
    commit_key = "looprail_commit" if schema == "looprail.bench.runtime-evidence.v2" else "source_commit"
    if schema not in {
        "looprail.bench.runtime-evidence.v2",
        "looprail.bench.runtime-scheduler-experiments.v1",
        "looprail.bench.runtime-live-scheduler.v2",
    }:
...
def _tokenwise_inputs(
    ...
    *,
    ...
    if report.get("schema") != "looprail.bench.tokenwise-cost.report.v1":
        raise ValueError("TokenWise report has the wrong schema")
    campaign = _mapping(report.get("campaign"), "TokenWise campaign")
    if campaign.get("looprail_commit") != looprail_commit:
        raise ValueError("TokenWise report does not match the current Looprail commit")
...
def _verify_current_digest_bound_evidence(
    ...
    *,
    ...
    if evidence.get(commit_key) != looprail_commit:
        raise ValueError("evidence does not match the current Looprail commit")
