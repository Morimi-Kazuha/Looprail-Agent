...
from looprail.providers.base import LLMProvider, ToolCallRequest
...
CAMPAIGN_SCHEMA = "looprail.bench.campaign.v1"
DEFAULT_SUITE_PATH = Path(__file__).resolve().parent / "suites" / "agent_application_ship_1.yaml"
DEFAULT_OUTPUT_ROOT = Path(".looprail/evidence/looprailbench")
_REPOSITORY_ROOT = Path(__file__).resolve().parents[2]
_REQUIRED_USAGE_FIELDS = frozenset(
    {"prompt_tokens", "completion_tokens", "total_tokens"},
)
_PROVIDER_PROBE_MAX_ATTEMPTS = 2
_PROVIDER_BUDGET_APPROVAL_SCHEMA = "looprail.bench.provider-budget-approval.v3"
_PROVIDER_PREFLIGHT_CACHE_SCHEMA = "looprail.bench.provider-preflight-cache.v1"
_RUNTIME_EVIDENCE_SCHEMA = "looprail.bench.runtime-evidence.v2"
...
class ResolvedProvider:
    ...
    looprail_config: object | None = None
...
class CampaignServices:
    ...
    resolve_looprail_commit: Callable[[], str]
...
async def _run_campaign(
    ...
    *,
    ...
    looprail_commit = _validated_looprail_commit(
        selected_services.resolve_looprail_commit(),
    )
    ...
    for paid_mode in paid_modes:
        if clean_worktree_required:
            _revalidate_formal_git_state(
                selected_services,
                expected_commit=looprail_commit,
            )
    ...
    campaign_artifact_path = (
        output_root / "campaigns" / suite_digest / looprail_commit / selected_mode.value / "campaign-outcome.json"
    )
...
def _exclusive_paid_campaign_lock(suite_digest: str) -> Iterator[None]:
    lock_path = get_product_home() / "evidence" / "looprailbench" / "campaign-locks" / f"{suite_digest}.lock"
    stack = ExitStack()
    try:
...
def default_campaign_services() -> CampaignServices:
    return CampaignServices(
        ...
        resolve_looprail_commit=_resolve_looprail_commit,
...
async def _run_deterministic_gate(
    ...
    from looprail.sandbox.config import SandboxConfig
    ...
    looprail_commit = _validated_looprail_commit(_resolve_looprail_commit())
    ...
    final_looprail_commit = _validated_looprail_commit(_resolve_looprail_commit())
    final_worktree_clean = _worktree_is_clean()
    if final_looprail_commit != looprail_commit or final_worktree_clean != worktree_clean:
        raise CampaignError(
    ...
    evidence_root = output_root / "runtime-evidence" / looprail_commit
...
def _resolve_looprail_commit() -> str:
    git = shutil.which("git")
    if git is None:
        raise CampaignError("git is required to bind LooprailBench evidence")
    completed = subprocess.run(
    ...
    if completed.returncode != 0:
        raise CampaignError("cannot resolve the current Looprail commit")
    return _validated_looprail_commit(completed.stdout.strip())


def _resolve_provider() -> ResolvedProvider:
    from looprail.cli._helpers import make_provider
    from looprail.config.loader import load_config
    from looprail.config.looprail import load_looprail_config

    config = load_config()
    looprail_config = load_looprail_config()
    ...
    return ResolvedProvider(
        ...
        runtime_config_digest=canonical_digest(
            _redacted_runtime_config(config, looprail_config),
        ),
        config=config,
...
def _prepare_budget_guard(
    ...
    approval_root = get_product_home() / "evidence" / "looprailbench" / "approvals" / approval_id
    ...
    *,
    ...
    try:
        ...
        payload = {
            ...
            "looprail_commit": str(approval["looprail_commit"]),
            "runtime_config_digest": str(
    ...
    if (
        ...
        or payload["looprail_commit"] != looprail_commit
...
def _build_registry(
    ...
    if resolved.config is None or resolved.looprail_config is None:
        raise CampaignError(
    ...
    memory_runner = RuntimeCrossProcessRunner(
        ...
        looprail_config=resolved.looprail_config,
        provider=provider,
    )
...
async def _probe_provider(
    ...
    tools = [
        {
            "type": "function",
            "function": {
                "name": "looprailbench_preflight_echo",
    ...
    def valid_probe_call(call: ToolCallRequest) -> bool:
        return (
            call.name == "looprailbench_preflight_echo"
            and isinstance(call.arguments, dict)
            and call.arguments.get("value") == "ready"
        )

    with provider_call_budget_scope(
        trial_id=(f"preflight/{request.looprail_commit}/{request.suite_digest}/{resolved.runtime_config_digest}"),
        max_logical_calls=1,
                    ...
                    messages=[
                        {
                            "role": "user",
                            "content": (
                                'Call looprailbench_preflight_echo exactly once with value "ready". Do not answer with text.'
                            ),
                        },
                    ],
                    tools=tools,
...
def _resolved_environment_identity(
    ...
    config = resolved.config
    if config is None and resolved.looprail_config is not None:
        config = getattr(resolved.looprail_config, "base", None)
...
def _validated_looprail_commit(value: str) -> str:
    commit = value.strip().lower()
    if len(commit) not in {40, 64} or any(character not in "0123456789abcdef" for character in commit):
        raise CampaignError("current Looprail commit is not a full Git object id")
    return commit
...
def _revalidate_formal_git_state(
    ...
    *,
    ...
    current_commit = _validated_looprail_commit(
        services.resolve_looprail_commit(),
    )
    ...
    try:
        ...
        token_count = litellm.token_counter(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "LooprailBench tokenizer preflight",
                },
            ],
        )
...
def _redacted_runtime_config(
    config: object,
    looprail_config: object,
) -> dict[str, Any]:
    return {
        "base": _redact_secrets(_model_dump(config)),
        "looprail": _redact_secrets(_model_dump(looprail_config)),
    }
