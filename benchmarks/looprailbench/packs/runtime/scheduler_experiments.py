...
SCHEDULER_EXPERIMENT_SCHEMA = "looprail.bench.runtime-scheduler-experiments.v1"
...
def _make_work(
    *,
    ...
    conversation_id = f"looprailbench-scheduler:{conversation}"
    ...
    return _Work(
        request=TurnRequest(
            origin=origin,
            source=Source(
                channel="looprailbench-scheduler",
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path(".looprail/evidence/looprailbench-runtime-scheduler"),
    )
...
async def _main() -> int:
    ...
    logger.disable("looprail.spine.scheduler")
    try:
        ...
        logger.enable("looprail.spine.scheduler")
