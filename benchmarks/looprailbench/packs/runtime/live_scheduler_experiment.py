...
from looprail.providers.base import GenerationSettings, LLMProvider
from looprail.spine import ChatType, Origin, OriginPools, Scheduler, Source, Text, TurnOutcome, TurnRequest
...
LIVE_SCHEDULER_SCHEMA = "looprail.bench.runtime-live-scheduler.v2"
LIVE_TURN_RECORD_SCHEMA = "looprail.bench.runtime-live-turn-record.v1"
_DISABLED_TOOLS = [
    "read_file",
    "write_file",
    "edit_file",
    ...
    *,
    ...
    return {
        ...
        "workload": {
            ...
            "treatment": "looprail_per_session_lanes",
...
async def _run_live_arm(
    ...
    *,
    ...
    looprail_config.plugins.disabled = []
    ...
    looprail_config.token_wise.enabled = False
...
def _live_work(config: LiveSchedulerConfig, repetition: int) -> list[_LiveWork]:
    ...
    for index in range(config.hot_turns):
        marker = f"LOOPRAIL_LIVE_PERF_R{repetition:02d}_HOT_{index:02d}"
    ...
    for index in range(config.foreground_sessions):
        marker = f"LOOPRAIL_LIVE_PERF_R{repetition:02d}_FG_{index:02d}"
        prompt = f"Reply with exactly {marker} and nothing else. Do not call tools."
...
def _make_work(marker: str, conversation: str, prompt: str, *, measured: bool) -> _LiveWork:
    return _LiveWork(
        request=TurnRequest(
            origin=Origin.USER,
            source=Source(
                channel="looprailbench-live-scheduler",
            ...
            text=prompt,
            message_id=marker,
            conversation=f"looprailbench-live-scheduler:{conversation}",
        ),
        marker=marker,
        measured=measured,
    )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path(".looprail/evidence/looprailbench-runtime-live-scheduler"),
    )
