...
from looprail.spine.delivery import (
    Capabilities,
    DeliveryHub,
    make_hub_sink,
)
...
class _DeterministicProvider(LLMProvider):
    ...
    def get_default_model(self) -> str:
        return "looprailbench/deterministic"


class _ProbeTool(Tool):
    @property
    def name(self) -> str:
        return "looprailbench_probe"
...
class _R1Recorder:
    ...
    def _check_identity(self, marker: str | None, conversation: str) -> None:
        if marker is None or conversation != f"looprailbench-r1:{marker}":
            self.identity_contradictions += 1
...
async def run_r1_full_runtime_track(root: Path) -> R1RuntimeResult:
    ...
    requests = {f"looprailbench-r1:r1-{index:03d}": _request(index) for index in range(R1_FULL_PATH_TURNS)}
...
def _request(index: int) -> TurnRequest:
    ...
    return TurnRequest(
        ...
        message_id=marker,
        conversation=f"looprailbench-r1:{marker}",
    )
...
def _provider_identity_contradictions(
    ...
    return sum(f"looprailbench-r1:{marker}" not in requests for marker in calls)
