...
from benchmarks.looprailbench.budget import ProviderBudgetLedger
from benchmarks.looprailbench.canonical import canonical_digest
from looprail.context_engine.base import AssemblyContext
from looprail.context_engine.segments import MemorySegmentBuilder, SkillsSegmentBuilder
...
from looprail.memory_engine.skill_local import LocalPool, SkillMeta
...
class SemanticRuntimeConfig:
    ...
    semantic_schema: str = "looprail.bench.semantic-addendum.v1"
    ...
    *,
    ...
    metas = [
        SkillMeta(
            ...
            name=item.logical_id,
            description=item.text,
            path=Path("/looprailbench-semantic") / item.item_id / "SKILL.md",
            content=item.text,
            source="local",
        )
        for item in corpus
...
def _assembly_context(query: str, consuming_turn: str) -> AssemblyContext:
    return AssemblyContext(
        ...
        media=None,
        channel="looprailbench-semantic",
