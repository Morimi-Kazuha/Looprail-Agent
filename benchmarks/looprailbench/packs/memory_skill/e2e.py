"""Historical EverOS-backed LooprailBench path retained without a current Runtime."""
...
from benchmarks.looprailbench.budget import BudgetGuardedProvider
...
class RuntimeCrossProcessRunner:
    ...
    async def run(self, context: TrialContext) -> TrialExecution:
        ...
        with tempfile.TemporaryDirectory(
            prefix="looprailbench-memory-skill-provider-",
        ) as private_root:
...
async def _run_cross_process_trial(
    ...
    *,
    ...
    isolation = TrialIsolation.create(
        context.experiment.output_root / ".looprailbench-memory-skill" / context.experiment_id,
        attempt_id,
    )
    isolation.prepare()
...
async def _run_stage(
    *,
    ...
    process = await asyncio.create_subprocess_exec(
        sys.executable,
        "-m",
        "benchmarks.looprailbench.packs.memory_skill.worker",
        "--stage",
        stage,
        "--spec",
        str(spec_path),
