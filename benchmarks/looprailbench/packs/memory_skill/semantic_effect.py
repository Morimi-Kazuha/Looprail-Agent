...
from benchmarks.looprailbench.budget import (
    BudgetGuardedProvider,
    ProviderBudgetConfig,
    ProviderBudgetLedger,
    provider_call_budget_scope,
)
from benchmarks.looprailbench.canonical import canonical_digest, to_primitive
from benchmarks.looprailbench.coverage import assess_pair_coverage
from benchmarks.looprailbench.host import RecordingOutlet, RuntimeTrialHost
from benchmarks.looprailbench.isolation import TrialIsolation
from benchmarks.looprailbench.protocol import TrialContext, TrialExecution
from benchmarks.looprailbench.records import (
    DeliveryOutcome,
    TrialStatus,
    TurnTerminalState,
    VerificationState,
...
from benchmarks.looprailbench.schema import (
    JsonValue,
    PackDefinition,
    PairSpec,
    TaskSpec,
...
from benchmarks.looprailbench.usage import (
    RecordingProvider,
    UsageRecorder,
    usage_scope,
)
...
from looprail.memory_engine import Memory
...
from looprail.utils.helpers import estimate_prompt_tokens

from .models import SemanticMemoryEffectTask
from .semantic_runtime import CountingEmbeddingProvider

_TASK_SCHEMA = "looprail.bench.semantic-memory-effect-tasks.v1"
...
class ScriptedSemanticMemoryEffectRunner:
    ...
    async def run(self, context: TrialContext) -> TrialExecution:
        return await _run_semantic_memory_effect_trial(
            ...
            provider_spec={
                "mode": "scripted",
                "provider_identity": "scripted/looprailbench-semantic-memory-effect",
...
class ProductionSemanticMemoryEffectRunner:
    ...
    def __init__(
        ...
        *,
        config: Config,
        looprail_config: LooprailConfig,
        ...
        self._config = config
        self._looprail_config = looprail_config
    ...
    async def run(self, context: TrialContext) -> TrialExecution:
        ...
        with tempfile.TemporaryDirectory(
            prefix="looprailbench-semantic-memory-private-",
        ) as private_root:
                ...
                json.dumps(
                    {
                        ...
                        "looprail_config": self._looprail_config.model_dump(mode="json"),
                    },
                    sort_keys=True,
                ),
                encoding="utf-8",
...
async def _run_semantic_memory_effect_trial(
    ...
    *,
    ...
    isolation = TrialIsolation.create(
        (context.experiment.output_root / ".looprailbench-semantic-memory-effect" / context.experiment_id),
        attempt_id,
    )
    isolation.prepare()
    ...
    *,
    ...
    environment = {
        ...
        **(private_environment or {}),
        ...
        "LOOPRAIL_TRACING_DIR": str(isolation.trace_root),
    }
    ...
    process = await asyncio.create_subprocess_exec(
        sys.executable,
        "-m",
        "benchmarks.looprailbench.packs.memory_skill.semantic_effect",
        "--worker-stage",
        stage,
        "--spec",
        str(spec_path),
...
class _RecordingProductionEverosBackend:
    def __init__(
        ...
        *,
        ...
        from looprail.plugin.memory.everos.backend import EverosBackend
    ...
    async def start(self) -> None:
        if self._drive_production_lifespan:
            ...
            return
        from looprail.plugin.memory.everos.backend import (
...
class _ScriptedSemanticEffectProvider(LLMProvider):
    def __init__(self, task: SemanticMemoryEffectTask) -> None:
        super().__init__(api_key="looprailbench")
        self._task = task
        self._calls = 0
        self.memory_observed = False

    def get_default_model(self) -> str:
        return "scripted/looprailbench-semantic-memory-effect"
...
async def _evaluation_stage(
    *,
    ...
    outlet = RecordingOutlet("looprailbench-semantic-memory-effect")
    conversation = f"{task.task_id}:evaluation"
    request = TurnRequest(
        origin=Origin.USER,
        source=Source(
            channel="looprailbench-semantic-memory-effect",
...
def _runtime_dependencies(
    *,
    ...
    if provider_spec["mode"] == "scripted":
        ...
        return (
            config,
            _freeze_looprail_config(
                LooprailConfig(base=config),
                config,
                task,
            ),
            delegate,
    ...
    looprail_config = _freeze_looprail_config(
        LooprailConfig.model_validate(payload["looprail_config"]),
        config,
        task,
    )
    ledger = ProviderBudgetLedger(
    ...
    return config, looprail_config, delegate, ledger, model
...
def _freeze_looprail_config(
    looprail_config: LooprailConfig,
    config: Config,
    task: SemanticMemoryEffectTask,
) -> LooprailConfig:
    looprail_config.base = config
    looprail_config.memory.backend = None
    looprail_config.memory.user_id = task.workspace_id
    looprail_config.memory.agent_id = task.workspace_id
    looprail_config.memory.memory_top_k = 5
    looprail_config.skill_forge.rewrite_enabled = False
    looprail_config.skill_forge.llm_gate_enabled = False
    looprail_config.token_wise.smart_routing.enabled = False
    looprail_config.runtime.checkpoint.policy = "never"
    return looprail_config
...
def _configured_embedding_environment() -> tuple[
    ...
    from everos.config import load_settings
    from looprail.config.update_everos import get_everos_config_path
