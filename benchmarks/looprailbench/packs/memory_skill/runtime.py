...
from benchmarks.looprailbench.usage import RecordingProvider, UsageRecorder, usage_scope
...
class ScriptedCrossSessionProvider(LLMProvider):
    ...
    def get_default_model(self) -> str:
        return "scripted/looprailbench-memory-skill"
...
async def run_runtime_stage(
    *,
    ...
    provider_spec = provider_spec or {
        "mode": "scripted",
        "provider_identity": "scripted/looprailbench-memory-skill",
    ...
    outlet = RecordingOutlet("looprailbench-memory-skill")
    conversation = f"{task.task_id}:{stage}"
    request = TurnRequest(
        origin=Origin.USER,
        source=Source(
            channel="looprailbench-memory-skill",
...
def _runtime_dependencies(
    *,
    ...
    if provider_spec["mode"] == "scripted":
        ...
        return (
            config,
            _freeze_looprail_config(LooprailConfig(base=config), config, task),
            delegate,
            None,
    ...
    looprail_config = LooprailConfig.model_validate(payload["looprail_config"])
    looprail_config = _freeze_looprail_config(looprail_config, config, task)
