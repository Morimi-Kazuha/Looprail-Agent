...
from benchmarks.looprailbench.fixtures.mcp import (
    MCP_CATALOG_SIZE,
    catalog_definitions,
    catalog_digest,
)
...
from looprail.config.schema import Config, MCPServerConfig
...
class _ScriptedToolMCPProvider(LLMProvider):
    ...
    def get_default_model(self) -> str:
        return "looprailbench/tool-mcp-deterministic"
...
async def _run_runtime_trial(
    *,
    ...
    disclosure = str(context.variant.settings["tool_disclosure"])
    isolation_root = context.experiment.output_root / ".looprailbench-tool-mcp" / context.experiment_id
    ...
    config, looprail_config = _runtime_config(
        workspace=isolation.workspace,
        receipt_path=receipt_path,
        provider=provider,
        progressive=disclosure == "progressive_disclosure",
    )
    outlet = RecordingOutlet("looprailbench-tool-mcp")
    started = time.perf_counter()
    observation = None
    ...
    with patch.dict("os.environ", environment, clear=False):
        ...
        try:
            ...
            catalog_payloads = [
                ...
                if definition["function"]["name"].startswith("mcp_looprailbench_catalog_probe_")
            ]
        finally:
            await host.close()
    latency_ms = (time.perf_counter() - started) * 1_000
...
async def run_mcp_transport_smoke(root: Path) -> MCPTransportSmokeResult:
    ...
    try:
        await connect_mcp_servers(
            {"looprailbench": _mcp_server_config(receipt_path)},
            registry,
            stack,
        )
...
def _runtime_config(
    *,
    ...
    config.tools.mcp_servers = {"looprailbench": _mcp_server_config(receipt_path)}
...
def _mcp_server_config(receipt_path: Path) -> MCPServerConfig:
    ...
    return MCPServerConfig(
        ...
        env={"LOOPRAIL_BENCH_MCP_RECEIPTS": str(receipt_path)},
        tool_timeout=10,
    )
...
def _turn_request(task: ToolMCPTask) -> TurnRequest:
    return TurnRequest(
        origin=Origin.USER,
        source=Source(
            channel="looprailbench-tool-mcp",
        ...
        conversation=f"looprailbench-tool-mcp:{task.task_id}",
    )
