...
from looprail.providers.base import (
    ErrorClassification,
    GenerationSettings,
    LLMProvider,
    LLMResponse,
    ...
    async def __call__(
        ...
        *,
        ...
        isolation = TrialIsolation.create(
            context.experiment.output_root / ".looprailbench-context" / context.experiment_id,
            attempt_id,
        )
        isolation.prepare()
        config, looprail_config = _trial_configs(
            self._config,
            self._looprail_config,
        ...
        outlet = RecordingOutlet("looprailbench-context")
...
def _trial_configs(
    ...
    *,
    ...
    trial_looprail = looprail_config.model_copy(deep=True)
    trial_looprail.base = trial_config
    trial_looprail.context.curator_model = model
    trial_looprail.context.protect_first_n = CONTEXT_BENCHMARK_PROTECT_FIRST_N
    trial_looprail.memory.backend = None
    trial_looprail.skill_forge.enabled = False
    trial_looprail.skill_forge.router.enabled = False
    trial_looprail.skill_forge.rewrite_enabled = False
    trial_looprail.skill_forge.llm_gate_enabled = False
    trial_looprail.token_wise.smart_routing.enabled = False
    trial_looprail.runtime.checkpoint.policy = "never"
    return trial_config, trial_looprail
...
def _conversation_id(task: ContextTask, context: TrialContext) -> str:
    return (
        f"looprailbench-context:{task.task_id}-{context.variant.variant_id}-"
...
def _turn_request(
    ...
    return TurnRequest(
        origin=Origin.USER,
        source=Source(
            channel="looprailbench-context",
            chat_id=conversation.removeprefix("looprailbench-context:"),
            sender_id="looprailbench",
