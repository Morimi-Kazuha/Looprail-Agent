...
from benchmarks.looprailbench.canonical import canonical_bytes, canonical_digest
...
MANIFEST_SCHEMA = "looprail.bench.tracing-overhead.manifest.v1"
BLOCK_SCHEMA = "looprail.bench.tracing-overhead.block.v1"
AGGREGATE_SCHEMA = "looprail.bench.tracing-overhead.aggregate.v1"
...
async def run_campaign(
    *,
    ...
    manifest = _manifest(looprail_commit, config)
    ...
    aggregate["looprail_commit"] = looprail_commit
    ...
    return verify_campaign(
        output_root=output_root,
        expected_looprail_commit=looprail_commit,
    )


def verify_campaign(*, output_root: Path, expected_looprail_commit: str) -> dict[str, Any]:
    ...
    checks = {
        ...
        "looprail_commit": manifest.get("looprail_commit") == expected_looprail_commit,
    ...
    rebuilt["looprail_commit"] = expected_looprail_commit
    ...
    verifier = {
        "schema": "looprail.bench.tracing-overhead.verifier.v1",
        "checks": checks,
    ...
    _write_json(
        ...
        {
            "schema": "looprail.bench.tracing-overhead.claim.v1",
...
async def _run_arm(
    *,
    ...
    previous_tracing = os.environ.get("LOOPRAIL_TRACING")
    previous_dir = os.environ.get("LOOPRAIL_TRACING_DIR")
    os.environ["LOOPRAIL_TRACING"] = "1" if enabled else "0"
    os.environ["LOOPRAIL_TRACING_DIR"] = str(trace_dir)
    tracing_spans._store = None
    try:
        with tempfile.TemporaryDirectory(prefix="looprail-tracing-benchmark-") as tmp:
            root = Path(tmp)
        ...
        tracing_spans._store = None
        _restore_env("LOOPRAIL_TRACING", previous_tracing)
        _restore_env("LOOPRAIL_TRACING_DIR", previous_dir)
...
def _assemble_runtime(root: Path, provider: _TracingProvider):
    ...
    looprail_config.call_efficiency.mode = "off"
    looprail_config.call_efficiency.enabled = False
    looprail_config.call_efficiency.usage_tracking = False
...
def _manifest(looprail_commit: str, config: TracingExperimentConfig) -> dict[str, Any]:
    if len(looprail_commit) != 40:
        raise TracingExperimentError("looprail_commit must be a full Git commit")
    payload = {
...
def _inventory(output_root: Path) -> dict[str, Any]:
    ...
    payload = {
        "schema": "looprail.bench.tracing-overhead.inventory.v1",
        "files": files,
    }
...
def _current_commit() -> str:
    ...
    if git is None:
        raise TracingExperimentError("git is required to freeze the Looprail commit")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path(".looprail/evidence/tracing-overhead-current"),
    )
    parser.add_argument("--looprail-commit")
...
async def _main() -> int:
    ...
    commit = args.looprail_commit or _current_commit()
    if args.mode == "plan":
        result = _manifest(commit, config)
        result["pairs"] = config.pairs
        result["turns"] = config.pairs * 2
    elif args.mode == "run":
        result = await run_campaign(output_root=args.output_root, looprail_commit=commit, config=config)
    else:
        result = verify_campaign(output_root=args.output_root, expected_looprail_commit=commit)
