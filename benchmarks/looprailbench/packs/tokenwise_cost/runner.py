...
from looprail.call_efficiency.models import CallRecord
...
from looprail.providers.base import GenerationSettings, LLMResponse
...
def load_deepseek_key(*, config_path: Path | None = None) -> str:
    ...
    path = config_path or Path.home() / ".looprail" / "config.json"
...
async def run_formal_campaign(
    *,
    ...
    if len(looprail_commit) != 40:
        raise CampaignError("looprail_commit must be a full Git commit")
    ...
    manifest = {
        "schema": "looprail.bench.call-efficiency-cost.manifest.v2",
    ...
    report["campaign"]["looprail_commit"] = looprail_commit
...
def verify_retained_campaign(
    *,
    ...
    checks = {
        "manifest_schema": manifest.get("schema") == "looprail.bench.call-efficiency-cost.manifest.v2",
        "manifest_digest": expected_plan_digest == canonical_digest(manifest_payload),
        "looprail_commit": manifest.get("looprail_commit") == looprail_commit,
    ...
    checks["raw_trial_schema"] = all(
        row.get("schema") == "looprail.bench.call-efficiency-cost.trial.v2" for row in raw_rows
    )
    ...
    rebuilt["campaign"]["looprail_commit"] = looprail_commit
    ...
    verifier = {
        "schema": "looprail.bench.call-efficiency-cost.verifier.v1",
        "checks": checks,
    ...
    _write_json(
        ...
        {
            "schema": "looprail.bench.call-efficiency-cost.claim.v1",
...
async def run_cache_preflight(
    *,
    ...
    report = {
        "schema": "looprail.bench.tokenwise-cost.preflight.v1",
        "model": config.model,
        "price_snapshot": PRICE_SNAPSHOT,
        "checks": checks,
...
async def execute_live_trial(
    ...
    *,
    ...
    looprail_config = LooprailConfig(base=runtime_config)
    ...
    looprail_config.call_efficiency.mode = "observe"
    looprail_config.call_efficiency.usage_tracking = True
    looprail_config.context.fast_path_threshold = 1.0
...
def _trial_payload(artifact: TrialArtifact, *, plan_digest: str) -> dict[str, Any]:
    return {
        "schema": "looprail.bench.call-efficiency-cost.trial.v2",
        "plan_digest": plan_digest,
...
def _build_inventory(root: Path) -> dict[str, Any]:
    ...
    payload = {
        "schema": "looprail.bench.call-efficiency-cost.inventory.v1",
        "artifacts": artifacts,
    }
