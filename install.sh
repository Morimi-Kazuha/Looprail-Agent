...
ensure_uv() {
  ...
  curl -fsSL "$LOOPRAIL_UV_INSTALL_URL" | sh
...
# 索引不可达时回退到固定版本。
latest_node_v22() {
  idx="$(curl -fsSL "$LOOPRAIL_NODE_MIRROR/index.json" 2>/dev/null || true)"
  ver="$(printf '%s' "$idx" | tr ',' '\n' | grep -o '"version":"v22\.[0-9.]*"' \
         | head -n1 | sed 's/.*"v/v/; s/"$//')"
  [ -n "$ver" ] && printf '%s' "$ver" || printf 'v22.20.0'
}

# 输出 Looprail 配置的首个私有 Node 二进制路径；不存在时返回非零。遍历 glob 可避免
...
ensure_node() {
  ...
  if pn="$(private_node_bin)"; then
    ok "Looprail private Node already present ($pn)"
    return
  fi
  ...
  url="$LOOPRAIL_NODE_MIRROR/${ver}/${pkg}.tar.gz"
  ...
  curl -fsSL "$LOOPRAIL_NODE_CHECKSUM_BASE/${ver}/SHASUMS256.txt" -o "$tmp/SHASUMS256.txt" \
  ...
  # Looprail 的 find_node() 会自动匹配 ~/.looprail/runtime/node-*/bin/node，因此无需
  # 修改 PATH。
}

# --- 3. 安装 Looprail ----------------------------------------------------------
install_looprail() {
  # 本地模式：从 Looprail 源码检出运行时，以可编辑模式安装工作树，符合开发者预期；
  # 否则从 Git 安装。
  script_dir="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
  if [ -f "$script_dir/pyproject.toml" ] && grep -q '^name = "looprail"' "$script_dir/pyproject.toml" 2>/dev/null; then
    info "Local Looprail source detected; editable install: $script_dir"
    ...
    if [ ! -f "$script_dir/ui-tui/dist/entry.js" ]; then
      ...
      if [ -n "$node_bin" ] && [ -x "$node_bin" ]; then
        ...
        if PATH="$node_dir:$PATH" command -v npm >/dev/null 2>&1; then
          ...
          ( cd "$script_dir/ui-tui" && PATH="$node_dir:$PATH" npm ci --registry "$LOOPRAIL_NPM_REGISTRY" && PATH="$node_dir:$PATH" npm run build )
        else
          warn "Found node but not npm; skipping TUI build; looprail may not work"
        fi
      else
        warn "No usable node found; skipping TUI build; looprail may not work"
      fi
    fi
    install_result=0
    UV_DEFAULT_INDEX="$LOOPRAIL_PYPI_INDEX" uv tool install --force -e "$script_dir[channels]" || install_result=$?
    if [ "$install_result" -ne 0 ]; then
      warn "Channel dependencies failed to install; installed base looprail only. Some channels stay unavailable (see: looprail channels list)."
    ...
    if [ "$install_result" -ne 0 ]; then
      warn "Channel dependencies failed to install; installed base looprail only. Some channels stay unavailable (see: looprail channels list)."
      UV_DEFAULT_INDEX="$LOOPRAIL_PYPI_INDEX" uv tool install --force "$wheel_source"
    fi
  fi
  # 确保后续 shell 的 PATH 包含 uv 工具目录 ~/.local/bin。
  uv tool update-shell || true
  ok "Looprail installed"
}
...
main() {
  ...
  printf '\n    \033[1mlooprail onboard --skip-memory\033[0m    # configure Provider and first Turn\n'
  printf '    \033[1mlooprail\033[0m            # enter the TUI\n'
  printf '    \033[1mlooprail run\033[0m -m "hello"\n\n'
