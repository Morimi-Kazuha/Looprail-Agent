...
from pico.utils.atomic_io import locked_append, read_utf8_with_incomplete_tail
...
def _decode_record_line(line: str) -> EffectRecord:
    ...
    if not isinstance(payload, Mapping):
        raise ValueError("effect journal record must be an object")
    if payload.get("schema") != "pico.effect.v1" or not _RECORD_KEYS.issubset(payload):
