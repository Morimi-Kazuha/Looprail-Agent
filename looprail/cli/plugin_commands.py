"""``looprail plugins`` — inspect installed memory / context plugins.

Reads ``LooprailConfig.plugins`` + the live :class:`PluginRegistry`
...
from looprail.cli._helpers import load_runtime_config
...
def register(app: typer.Typer) -> None:
    ...
    def plugins(
        config_path: Optional[str] = typer.Option(
            ...
            "--config",
            "-c",
            help="Path to config file (default: ~/.looprail/config.json)",
        ),
        ...
        # 延迟导入，避免每次调用 ``looprail --help`` 都承担插件发现成本。
...
def _render_plugin_table(
    ...
    *,
    ...
    if not discovered:
        console.print(
            "[yellow]No plugins discovered.[/yellow] Reinstall Looprail from the "
            "same distribution source, or run [bold]uv sync[/bold] from a "
            "Looprail source checkout. Set "
            "[bold]memory.backend[/bold] to [bold]null[/bold], or drop a "
            "third-party manifest under [bold]~/.looprail/plugins/[/bold].",
        )
        return

    table = Table(
        title="Looprail plugins",
        show_header=True,
...
def _source_label(source) -> str:
    ...
    from looprail.plugin import Source
