...
def warn_about_pending_cli_reminders(cron_service, config: Config) -> None:
    ...
    if not config.cron.forward_channels:
        console.print(
            ...
            "`looprail cron config set forward_channels '*'` to broadcast to "
...
def check_provider_credentials(config: Config) -> None:
    ...
    if provider_name == "azure_openai":
        if not p or not p.api_key or not p.api_base:
            ...
            console.print("Set them in ~/.looprail/config.json under providers.azure_openai section")
    ...
    if not model.startswith("bedrock/") and not (p and p.api_key) and not (spec and (spec.is_oauth or spec.is_local)):
        ...
        console.print("Set one in ~/.looprail/config.json under providers section")
        raise typer.Exit(1)
...
def print_probe_troubleshooting(provider: str | None) -> None:
    ...
    if provider:
        console.print(
            f"  [dim]·[/dim] [cyan]looprail provider test {provider}[/cyan] — re-check credentials without spending tokens"
        )
        console.print(
            f"  [dim]·[/dim] [cyan]looprail provider get {provider}[/cyan] — inspect what's actually stored on disk"
        )
    console.print(
        "  [dim]·[/dim] Check the model id in [cyan]~/.looprail/config.json[/cyan] "
...
def print_deprecated_memory_window_notice(config: Config) -> None:
    ...
    if config.agents.defaults.should_warn_deprecated_memory_window:
        console.print(
            ...
            "[cyan]looprail onboard[/cyan] to refresh your config template."
        )
