...
writes its logs next to its own config rather than always to ``~/.looprail``.

Env vars:
    LOOPRAIL_CLI_DEBUG=1  — additionally mirror DEBUG+ to stderr.
"""
...
def redirect_loguru_to_file(
    ...
    *,
    ...
    if os.environ.get("LOOPRAIL_CLI_DEBUG"):
