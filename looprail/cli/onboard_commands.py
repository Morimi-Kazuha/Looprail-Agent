...
minutes, without ever opening ``~/.looprail/config.json`` or
...
from looprail.cli._helpers import (
...
def _pick_language() -> None:
    ...
    from looprail.cli._styles import LOOPRAIL_STYLE
    ...
    console.print(
        Panel(
            "[bold white]Let's set up Looprail — first, choose your language.[/bold white]\n"
            "[dim]开始配置 Looprail — 请先选择语言。[/dim]",
            title="[bold #fbe23f]Looprail setup[/bold #fbe23f]",
            title_align="left",
            border_style="#c8a900",
            padding=(1, 2),
        )
...
def _check_tty_or_die(non_interactive: bool) -> None:
    ...
    if not sys.stdout.isatty():
        console.print(
            ...
            "Re-run with: "
            "[#fbe23f]looprail onboard --non-interactive --provider <name> --api-key <key>[/#fbe23f]"
        )
        raise typer.Exit(2)
...
def _load_raw_config() -> dict[str, Any]:
    ...
    a syntax typo.
    """
    from looprail.config.loader import get_config_path, read_raw_or_raise
...
def _handle_existing_config(*, reset: bool, yes: bool, non_interactive: bool) -> None:
    ...
    if non_interactive:
        ...
        console.print(
            ...
            "[#fbe23f]looprail provider set[/#fbe23f] / [#fbe23f]looprail channels enable[/#fbe23f]."
        )
        raise typer.Exit(2)
...
def _bootstrap_empty_config() -> None:
    """Make sure ``~/.looprail/config.json`` + workspace dir exist before we patch.
    ...
    """
    from looprail.config.loader import get_config_path, load_config, save_config
    from looprail.config.paths import get_workspace_path
    from looprail.utils.helpers import sync_workspace_templates
...
def _validate_provider_name(name: str) -> str:
    ...
    from looprail.config.update_providers import provider_field_specs
...
def _run_oauth_login(provider: str) -> bool:
    ...
    """
    from looprail.cli.provider_commands import _LOGIN_HANDLERS
    from looprail.providers.registry import find_by_name
...
def _verify_provider(provider: str, *, skip_test: bool = False) -> tuple[bool, str, Optional[list[str]]]:
    ...
    """
    from looprail.config.update_providers import test_provider as probe
    ...
    hint_map = {
        ...
        "oauth_token_missing": _t(
            f"Run: looprail provider login {provider.replace('_', '-')}",
            f"请运行:looprail provider login {provider.replace('_', '-')}",
        ),
    }
...
def _run_test_probe(provider: str, *, non_interactive: bool, warnings: list[str], allow_repick: bool = True) -> str:
    ...
    try:
        ...
        console.print(
            _t(
                "  [dim]Run 'looprail provider test' to re-check, or confirm the model is served by this provider.[/dim]",
                "  [dim]可运行 'looprail provider test' 复查,或确认该模型确由此服务商提供。[/dim]",
            )
        )
    ...
    if is_oauth:
        if non_interactive:
            console.print(
                ...
                "Run [#fbe23f]looprail provider login "
...
def _step1_provider(
    *,
    ...
    console.print(
        _t(
            "  [dim]Looprail's chat and reasoning are all driven by it.[/dim]",
            "  [dim]Looprail 的对话与思考都由它驱动。[/dim]",
        )
    )
...
def _step3_sandbox(*, skip: bool, non_interactive: bool) -> object:
    ...
    _step_header(3, _t("Choose where Looprail runs code / commands", "选择 Looprail 运行代码 / 命令的位置"))
    ...
    picked = questionary.select(_t("Run location:", "运行位置:"), choices=choices, style=LOOPRAIL_STYLE, qmark=_QMARK).ask()
    if picked is None:
        raise typer.Exit(1)
    if picked is _BACK:
        return _BACK
        ...
        if reason == "missing":
            console.print(
                _t(
                    ...
                    "uv tool install --force 'looprail\\[channels,sandbox]'[/dim]",
...
def _ordered_channel_names() -> list[str]:
    from looprail.channels.registry import discover_channel_names
...
def _select_channel() -> Optional[str]:
    ...
    choices.append(questionary.Choice(_t("Back", "返回"), value=_BACK))
    picked = questionary.select(_t("Channel:", "渠道:"), choices=choices, style=LOOPRAIL_STYLE, qmark=_QMARK).ask()
    return picked
        ...
        while True:
            if spec.get("is_secret"):
                value = questionary.password(
                    prompt_label, placeholder=placeholder, style=LOOPRAIL_STYLE, qmark=_QMARK
                ).ask()
            else:
                value = questionary.text(prompt_label, placeholder=placeholder, style=LOOPRAIL_STYLE, qmark=_QMARK).ask()
            if value is None:
                raise typer.Exit(1)
            value = value.strip()
            if value:
...
def _enable_channel(channel: str, fields: dict[str, Any]) -> None:
    ...
    from looprail.config.update_channels import enable_channel
...
def _channel_uses_interactive_login(channel: str) -> bool:
    ...
    try:
        from looprail.channels.registry import discover_specs
...
def _scancode_login(channel: str, *, non_interactive: bool = False) -> None:
    ...
    Mirrors ``looprail channels login``: enable the channel so its config section
    ...
    from looprail.channels.registry import discover_specs
    from looprail.config.update_channels import disable_channel
    ...
    try:
        while True:
            ...
            # 向导会静音 Looprail 日志以保持 UI 整洁，但扫码登录通过 loguru 输出二维码、链接、进度和
            # 失败原因。登录尝试期间只重新启用当前渠道的适配器子树（而非整个 Looprail，以免输出
            # 无关噪声），随后恢复静音。
            _login_log_scope = f"looprail.channels.adapters.{channel}"
            try:
            ...
            console.print(
                _t(
                    f"  [dim]{channel} not connected — finish later with looprail channels login {channel}.[/dim]",
                    f"  [dim]{channel} 未接入 — 之后用 looprail channels login {channel} 完成。[/dim]",
                )
            )
            return
    finally:
        if not logged_in:
            ...
            # 扫码不会持久化为“已连接”。配置段仍保留，供 `looprail channels login` 使用。
            disable_channel(channel)
...
def _manage_existing_channels() -> None:
    ...
    questionary = _require_questionary()
    from looprail.cli._styles import LOOPRAIL_STYLE
    from looprail.config.update_channels import disable_channel, set_channel_fields
        ...
        if action == "edit":
            ...
            console.print(
                _t(
                    ...
                    f"with looprail channels enable {target}).[/green]",
                    f"  [green]✓ 已停用 {target}(凭证保留;之后用 looprail channels enable {target} 重新启用)。[/green]",
                )
            )
...
def _step4_channel(*, channel: Optional[str], skip: bool, non_interactive: bool) -> object:
    ...
    _step_header(
        4,
        _t(
            "(Optional) Connect a messaging app so you can chat with Looprail there",
            "(可选)接入即时通讯软件,直接在里面和 Looprail 聊天",
        ),
    )
    ...
    if non_interactive:
        if channel:
            console.print(
                ...
                f"Run [#fbe23f]looprail channels enable {channel} --<field> <value> ...[/#fbe23f] "
        ...
        if not enabled:
            action = questionary.select(
                ...
                choices=[
                    ...
                    questionary.Choice(
                        _t(
                            "Skip (add later with looprail channels enable)",
                            "跳过(之后用 looprail channels enable 添加)",
                        ),
                        value="skip",
                    ),
                ],
...
def _init_extension_block_defaults() -> None:
    ...
    from looprail.config.update import init_extension_block_defaults
...
def _prepare_myna(*, non_interactive: bool, yes: bool) -> bool:
    ...
    from looprail.cli._plugin_stack import inspect_memory_backend
    from looprail.config.looprail import load_looprail_config

    status = inspect_memory_backend(load_looprail_config())
...
def _step2_memory(
    *,
    ...
    if skip_test:
        console.print(
            _t(
                "  [dim]First Turn skipped via --skip-test; run looprail doctor --probe later.[/dim]",
                "  [dim]已通过 --skip-test 跳过首次 Turn;稍后可运行 looprail doctor --probe。[/dim]",
            )
        )
        return None
...
def _print_next_steps(*, warnings: list[str]) -> None:
        ...
        console.print(
            Panel(
                ...
                + _t(
                    ...
                    "(re-run [/dim][#fbe23f]looprail onboard[/#fbe23f][dim] to reconfigure).[/dim]",
                    "[dim]在依赖相关功能前请先修复(重新运行 [/dim][#fbe23f]looprail onboard[/#fbe23f][dim] 重新配置)。[/dim]",
                ),
                border_style="yellow",
                padding=(1, 2),
            )
    ...
    table.add_row("looprail", _t("launch the native TUI (default)", "启动原生 TUI(默认)"))
    table.add_row("looprail gateway", _t("run the gateway (serve channels)", "运行网关(对接渠道)"))
    table.add_row('looprail run -m "hello, world"', _t("ask a one-shot question", "一次性提问"))
    table.add_row("looprail channels list", _t("see connected chat channels", "查看已接入的渠道"))
    table.add_row("looprail provider list", _t("check your provider config", "检查当前服务商配置"))
    console.print(
        Panel(
            table,
...
def _run_wizard_body(
    *,
    ...
    if not non_interactive:
        from looprail.config.update import set_language
    ...
    console.print(
        Panel(
            _t(
                "[bold #fbe23f]✨ Welcome to the Looprail setup wizard[/bold #fbe23f]\n\n"
                ...
                "[bold #fbe23f]✨ 欢迎使用 Looprail 配置向导[/bold #fbe23f]\n\n"
                "[dim]我们将依次配置:[/dim]\n"
...
# ---------------------------------------------------------------------------
# 启动门——由裸 `looprail`、`looprail run` 和 TUI 入口调用
# ---------------------------------------------------------------------------
