...
_MYNA_BACKEND = "myna"
# This is Myna's third-party integration namespace; it is not Looprail branding.
        ...
        try:
            ...
            if getattr(exc, "code", None) in {"configuration_invalid", "myna_not_initialized"}:
                raise MynaSetupError(
                    "Myna is not initialized or its configuration is invalid; run 'myna init' in the Looprail "
...
def plugin_discovery_sources() -> dict:
    """Resolve the trusted discovery sources scanned by Looprail hosts.

    Shared by :func:`build_plugin_registry` (live boot) and the
    ``looprail plugins`` CLI command so both see the same set:

    - bundled - ``looprail/plugin/memory/`` inside the package;
    - user - operator-managed ``~/.looprail/plugins/``;
    - entry points - installed distributions in the ``looprail.plugins`` group.

    ``project_dir`` remains explicit and disabled. A checkout must not gain an
    executable startup hook merely by containing ``.looprail/plugins``.
    """
    import looprail

    return {
        "bundled_dir": Path(looprail.__path__[0]) / "plugin" / "memory",
        "user_dir": get_product_home() / "plugins",
        "project_dir": None,
        "entry_points_group": "looprail.plugins",
    }


def build_plugin_registry(
    config: "LooprailConfig",
) -> PluginRegistry:
    ...
    - **bundled** — ``looprail/plugin/memory/<id>/`` shipped inside the
      Looprail package.
    - **user** - ``~/.looprail/plugins/<id>/`` drop-in directories.
    - **entry_points** - the ``looprail.plugins`` group, where
...
def inspect_memory_backend(config: "LooprailConfig") -> MemoryBackendStatus:
...
def _validate_myna_identity(registry: PluginRegistry) -> None:
    try:
        ...
        raise PluginNotFoundError(
            ...
            "'myna init' in the Looprail workspace, or set memory.backend to null"
        ) from exc
