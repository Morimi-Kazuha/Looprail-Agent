"""``looprail session`` subapp - manage conversation sessions.
...
def session_create(
    ...
    The id materialises on first use when ``looprail run --session`` is
    called with it.
    ...
    if title is not None:
        ...
        console.print(f"  Use with: looprail run --session {key}")
    else:
        console.print(chat_id)
        console.print(f"[dim]  (lazy — materialises on first use: looprail run --session {key})[/dim]")
...
def session_resume(
    ...
    The printed key can be fed directly to ``looprail run --session``.
    This command is a pure resolver — it does not start an agent loop.
    """
    manager = _open_manager()
    key = resolve_session(manager, id_or_prefix)
    console.print(key)
    console.print(f"[dim]  Use with: looprail run --session {key}[/dim]")
...
def session_fork(
    ...
    the new child id (first line) for use with ``looprail run --session``.
    """
    manager = _open_manager()
    key = resolve_session(manager, id_or_prefix)
    ...
    console.print(f"[dim]  (forked from {_bare_id(key)}; use: looprail run --session {child.key})[/dim]")
