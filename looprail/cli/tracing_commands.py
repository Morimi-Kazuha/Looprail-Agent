"""``looprail tracing`` — open the tracing dashboard.

The dashboard is a dependency-free Node viewer bundled under
``looprail/tracing/viewer/``. Instrumentation itself runs in-process (installed at
CLI startup, see :mod:`looprail.tracing`); this command only launches the viewer
that reads the captured spans from ``~/.looprail/traces``.

``looprail tracing`` (bare) lazily starts the viewer if it is not already running,
then opens the browser. It reuses Looprail's Node discovery (:func:`find_node`),
so it needs the same Node >= 22 that ``looprail`` already requires.
