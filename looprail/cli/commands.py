...
- Top-level commands (each exposes a ``register(app)`` function):
    - ``run``      → ``looprail/cli/agent_commands.py``
    - ``doctor``   → ``looprail/cli/doctor_commands.py``
    - ``evolve``   → ``looprail/cli/evolve_commands.py``
    - ``gateway``  → ``looprail/cli/gateway_commands.py``
    - ``onboard``  → ``looprail/cli/onboard_commands.py``
    - ``status``   → ``looprail/cli/status_commands.py``

- Subcommand groups (each exposes a typer ``*_app`` instance):
    - ``channels`` → ``looprail/cli/channel_commands.py``
    - ``cron``     → ``looprail/cli/cron_commands.py``
    - ``provider`` → ``looprail/cli/provider_commands.py``
    - ``sessions`` → ``looprail/cli/session_commands.py``
    - ``skills``   → ``looprail/cli/skill_commands.py``

Shared helpers used across multiple command modules live in
``looprail/cli/_helpers.py``.
"""
...
from looprail import __logo__, __version__

app = typer.Typer(
    name="looprail",
    help=f"{__logo__} Looprail - compact, composable, benchmark-gated Agent Harness with an opt-in evolution path",
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} Looprail v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """Looprail - compact, composable, benchmark-gated Agent Harness with an opt-in evolution path.

    Bare ``looprail`` runs the startup gate and enters the native TUI.
    """
    if ctx.invoked_subcommand is not None:
        return
    from looprail.cli.tui_commands import launch_tui
...
from looprail.cli.channel_commands import channels_app
from looprail.cli.cron_commands import cron_app
from looprail.cli.provider_commands import provider_app
from looprail.cli.sandbox_commands import sandbox_app
from looprail.cli.skill_commands import skill_app
...
from looprail.cli.session_commands import session_app
...
def run() -> None:
    ...
    Rust/tokio background thread — see :mod:`looprail.cli._exit`). Any command that
