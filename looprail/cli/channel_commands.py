...
This module bundles all ``looprail channels ...`` subcommands:
...
:mod:`looprail.config.update_channels`. Command bodies do not import
...
def _parse_channel_flags(extra_args: list[str], channel_name: str) -> dict:
    ...
    :func:`looprail.config.update_channels.channel_field_specs`. Unknown
    ...
    while i < len(extra_args):
        ...
        if flag.startswith("no-") and value is None:
            ...
            if key not in specs:
                raise typer.BadParameter(
                    f"Unknown field '--no-{flag[3:]}'. Run 'looprail channels show {channel_name}' for available flags."
                )
            out[key] = False
            continue
        ...
        if key not in specs:
            raise typer.BadParameter(
                ...
                f"Run 'looprail channels show {channel_name}' for available flags."
            )
...
def _register_config_commands(channels_app: typer.Typer) -> None:
    ...
    Call once at module-init time from ``looprail/cli/commands.py`` after
    ...
    """
    # `looprail channels`（无子命令）应打印帮助，而不是 "Missing command"。
    ...
    if "channels list" not in base_help:
        channels_app.info.help = f"{base_help}\n\nRun 'looprail channels list' to see available channels."
    ...
    def channels_enable_cmd(
        ...
        Examples:
            looprail channels enable feishu --app-id X --app-secret Y
            looprail channels enable qq --app-id X --secret Y
        """
    ...
    def channels_set_cmd(
        ...
        Examples:
            looprail channels set feishu --group-policy open
            looprail channels set wecom --welcome-message "Hello"
        """
        if _help_requested(ctx.args):
            _print_schema_table(name)
            raise typer.Exit(0)
        ...
        from looprail.config.update_channels import set_channel_fields
    ...
    def channels_get_cmd(
        ...
        from looprail.config.update_channels import get_channel_config
    ...
    def channels_reset_cmd(
        ...
        """
        from looprail.config.update_channels import get_channel_config, reset_channel
...
# ---------------------------------------------------------------------------
# channels_app：`looprail channels ...` 的顶层 Typer 命令组
# ---------------------------------------------------------------------------
...
def channels_login(
    ...
    if not spec.capabilities.interactive_login:
        console.print(
            ...
            f"Configure it with: [cyan]looprail channels set {channel_name}[/cyan]"
        )
        return
