"""``looprail doctor`` — health check (static + optional --probe).

Default mode is zero-network, millisecond-fast. ``--probe`` sends one
chat exchange via :func:`looprail.cli._helpers.send_probe`.
...
from looprail.cli._helpers import print_probe_troubleshooting, send_probe
...
def _gather_static_checks() -> DoctorReport:
    ...
    memory = inspect_memory_backend(load_looprail_config(config_path))
    ...
    from looprail.cli._gateway_lock import read_status
...
def _render_human_output(report: DoctorReport) -> None:
    ...
    if not paths.config_exists:
        console.print("\n[yellow]⚠ Looprail is not configured.[/yellow] Run [cyan]looprail onboard[/cyan] to set it up.")
        return

    if not report.config_loaded:
        console.print("\n[red]✗ Config schema invalid.[/red] Run [cyan]looprail onboard --reset[/cyan] to recreate it.")
        return
    ...
    if code == 0:
        ...
        console.print("Run [cyan]looprail provider list[/cyan] / [cyan]looprail provider set[/cyan] to fix routing.")
        ...
        console.print("[red]✗ Native TUI bundle is missing.[/red] Reinstall Looprail or rebuild the TUI.")
...
def register(app: typer.Typer) -> None:
    ...
    def doctor(
        ...
        """Health-check Looprail config, routing, and (optionally) the LLM."""
