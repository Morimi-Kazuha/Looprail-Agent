"""Local slash-command handling for the interactive ``looprail run`` REPL.
...
``looprail cron`` CLI command functions directly
...
``~/.looprail/config.json``. Every config write is shell-only, including
...
_CRON_SHELL_ONLY = {
    ...
    "run inside the REPL event loop. Use `looprail cron run` in a shell.",
}
...
def _handle_cron(args: list[str], console: "Console") -> bool:
    from looprail.cli import cron_commands as cc
    ...
    if sub == "config":
        if rest and rest[0] in ("set", "reset"):
            console.print(
                ...
                "config). Use `looprail cron config ...`.[/yellow]"
            )
            return True
        _invoke(
            console,
...
def _print_cron_help(console: "Console") -> None:
    console.print(
        "[bold]/cron[/bold] — scheduled jobs (~/.looprail/cron/jobs.json)\n"
