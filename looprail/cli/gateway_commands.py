...
Spawns the Looprail gateway: agent loop + channel manager + cron service.
...
def build_model_routing(config, provider):
    ...
    from looprail.routing.types import RoutingProfileName
...
def _build_gateway_channels(config) -> set[str]:
    ...
    them (the TUI / ``looprail run`` session), so a TUI-set reminder always
...
def register(app: typer.Typer) -> None:
    ...
    def gateway(
        ...
        from looprail.config.paths import get_cron_dir, resolve_service_paths
        from looprail.proactive_engine.schedulers.cron.service import CronService

        # load_runtime_config 必须最先运行：它调用 set_config_path()，使后续 load_looprail_config()
        ...
        from looprail.cli._gateway_lock import GatewayAlreadyRunningError, acquire
        ...
        try:
            ...
            console.print(
                f"[red]✗[/red] Looprail gateway already running for this instance "
        ...
        console.print(f"{__logo__} Starting Looprail gateway on port {port}...")
        ...
        async def run():
            ...
            try:
                ...
                # 线程地址，而无需从会话 ID 重建。
                from looprail.spine import Text as _Text
                ...
                from looprail.spine import Text
                from looprail.spine.turn import BusyPolicy
