"""``looprail cron`` subapp — operator-facing CLI for the CronService
state at ``~/.looprail/cron/jobs.json``.
...
- ``cron add`` stores ``channel="cli"`` / ``to="direct"`` by default
  ...
  ``cron.forward_channels`` (see ``looprail cron config``).
"""
...
from looprail.cli._log_silence import mute_subsystem_logs_unless_debug
from looprail.config.paths import get_cron_dir
from looprail.proactive_engine.schedulers.cron.service import CronService
from looprail.proactive_engine.schedulers.cron.types import CronJob, CronSchedule

cron_app = typer.Typer(
    help="Inspect and manage scheduled cron jobs (~/.looprail/cron/jobs.json)",
...
def _suppress_info_logs() -> None:
    """Mute Looprail subsystem INFO logs so the CLI table output stays
    clean. Set ``LOOPRAIL_CLI_DEBUG=1`` to keep all logs."""
...
def cron_add(
    ...
    For interactive use, prefer ``looprail run`` — the LLM understands
    ...
    ``looprail cron config get`` for current routing.
    """
    # 校验调度方式：三者必须且只能选择一个。
    schedule_flags = [
        ("--cron", cron),
...
cron_config_app = typer.Typer(
    help="Read / write the cron section of ~/.looprail/config.json",
    no_args_is_help=True,
)
...
def cron_config_set(
    ...
    from looprail.config.update import update_cron_config
...
def cron_config_reset(
    ...
    from looprail.config.update import reset_cron_config
