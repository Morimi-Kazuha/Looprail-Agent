"""Native TUI launcher for the bare ``looprail`` entry point.
...
from looprail.cli._log_file import _strip_tty_stream_handlers, redirect_loguru_to_file, redirect_terminal_fds_to_file
from looprail.product import get_product_home
from looprail.spine._barrier import finish_barrier

# 相对于本文件的 ui-tui/ 源码树路径：
# looprail/cli/tui_commands.py -> ../../../ui-tui/。仅 `--dev` 路径（从源码运行 tsx）需要它；
# 该路径依赖 src/ 和 node_modules，wheel 中不存在。
_UI_TUI_DIR = Path(__file__).resolve().parent.parent.parent / "ui-tui"

# 已安装 wheel 内预构建、自包含包的位置：looprail/cli/tui_commands.py ->
# ../ui-tui/dist/entry.js（即 looprail/ui-tui/dist/entry.js）。pyproject 会强制把
...
def resolve_dist_entry() -> Optional[Path]:
    ...
    Tries, in order:
      1. The packaged copy inside the installed wheel (``looprail/ui-tui/dist``).
...
def find_node() -> Tuple[Optional[str], Optional[Tuple[int, int, int]]]:
    ...
    """
    # 优先级 1：LOOPRAIL_NODE 环境变量——显式覆盖，不回退。用户设置 LOOPRAIL_NODE 即表示强制
    # 使用指定二进制；若它缺失或不可用，绝不能静默回退到虚拟环境或 PATH，否则会掩盖错误配置。
    candidates: list[str] = []
    if env_node := os.environ.get("LOOPRAIL_NODE"):
        ...
        # 优先级 4：一行安装器安装到 ~/.looprail/runtime/ 的 Looprail 私有运行时。这是零配置回退，
        # 即使用户没有系统 Node，安装器在此配置私有 Node 后仍可正常运行 `looprail`。使用 glob
...
# Node 子进程连接 LOOPRAIL_RPC_SOCKET 导出的 TCP 回环地址，并使用 LOOPRAIL_RPC_TOKEN 共享密钥
# 认证。任何本地进程都能访问回环地址，因此服务器分发请求前需由令牌维持信任边界。
_RPC_SOCKET_ENV: str = "LOOPRAIL_RPC_SOCKET"
_RPC_TOKEN_ENV: str = "LOOPRAIL_RPC_TOKEN"  # noqa: S105 -- 这是环境变量名，不是密钥值
...
def _build_tui_runtime():
    ...
    try:
        from looprail.cli._helpers import load_runtime_config, make_lazy_provider
        from looprail.cli._runtime_assembly import assemble_runtime
        from looprail.config.looprail import load_looprail_config
        from looprail.config.paths import get_cron_dir, resolve_foreground_paths
        ...
        raise InternalError(
            ...
            data={
                ...
                "log_path": "~/.looprail/logs/tui.log",
...
async def _run_rpc_server_until_done(
    ...
    try:
        ...
        from looprail.config.paths import get_logs_dir
...
def _spawn_with_rpc_socket(
    ...
    Topology (cross-platform: macOS / Linux / Windows):
        ...
        parent: socket()/bind()/listen() on 127.0.0.1:<ephemeral>; exports the
                ``host:port`` as ``LOOPRAIL_RPC_SOCKET`` and a random shared secret
                as ``LOOPRAIL_RPC_TOKEN``
        child:  reads ``LOOPRAIL_RPC_SOCKET``, ``net.createConnection({host,port})``,
                sends ``LOOPRAIL_RPC_TOKEN`` as the first line, then emits JSON-RPC
...
def run_subprocess_with_rpc(
    ...
    ``LOOPRAIL_RPC_SOCKET`` plus a random shared secret via ``LOOPRAIL_RPC_TOKEN``.
...
def _print_node_help(out=None) -> None:
    ...
    msg = (
        ...
        "  或：使用当前支持的行式 Agent 入口  ->  looprail run --help\n"
    )
...
def launch_tui(
    *,
    ...
    if not (check or print_colors or preview_colors) and _stdout_isatty():
        from looprail.cli.onboard_commands import (
    ...
    # 避免 "auto" 默认值覆盖 shell 层的 LOOPRAIL_TUI_COLOR。
    if color is not None:
        os.environ["LOOPRAIL_TUI_COLOR"] = color

    # `--check` 是冒烟测试：让子进程启动并证明运行时初始化成功，然后以 0 退出
    # （不渲染 Ink，也不需要交互 TTY）。子进程从继承的环境中读取 LOOPRAIL_TUI_CHECK；
    # 对应处理器见 ui-tui/src/entry.tsx。
    if check:
        os.environ["LOOPRAIL_TUI_CHECK"] = "1"

    # `--print-colors` / `--preview-colors` 是无 IPC 诊断：子进程输出解析后的调色板
    # （色块/上下文预览）后退出。它们与 --check 一样跳过 RPC 握手。
    if print_colors:
        os.environ["LOOPRAIL_TUI_PRINT_COLORS"] = "1"
    if preview_colors:
        os.environ["LOOPRAIL_TUI_COLOR_PREVIEW"] = "1"
    ...
    if dev:
        # 通过本地 node_modules 运行 tsx。根据已验证的 node_path 推导 npx，使 LOOPRAIL_NODE 的
        # 版本固定语义端到端生效。仅当推导路径不存在时才回退到 PATH（少见，例如运维人员
        # 将 LOOPRAIL_NODE 指向没有同级 npx 的独立 node 二进制）。
        derived_npx = Path(node_path).parent / "npx"
        if derived_npx.exists():
            npx = str(derived_npx)
        else:
            fallback = shutil.which("npx")
            if fallback is not None and os.environ.get("LOOPRAIL_NODE"):
                typer.echo(
                    f"⚠ LOOPRAIL_NODE was set but no `npx` next to {node_path};\n"
        ...
        # LOOPRAIL_TUI_CHECK 短路，无需 RPC 服务器；交互路径必须打开套接字，否则 entry.tsx
        # 以状态 2 退出（"LOOPRAIL_RPC_SOCKET env var required"）。
