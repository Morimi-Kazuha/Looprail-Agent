...
:mod:`looprail.config.update_providers`. Command bodies do not import
...
def provider_login(
    ...
    from looprail.providers.registry import PROVIDERS
...
def _parse_provider_flags(extra_args: list[str], provider_name: str) -> dict[str, Any]:
    ...
    Mirrors ``_parse_channel_flags`` (looprail/cli/channel_commands.py:109) — the
    ...
    while i < len(extra_args):
        ...
        if flag.startswith("no-") and value is None:
            ...
            if key not in specs:
                raise typer.BadParameter(
                    f"Unknown field '--no-{flag[3:]}'. Run 'looprail provider show {provider_name}' for available flags."
                )
            out[key] = False
            continue
        ...
        if key not in specs:
            raise typer.BadParameter(
                ...
                f"Run 'looprail provider show {provider_name}' for available flags."
            )
...
def _register_config_commands(app: typer.Typer) -> None:
    ...
    def provider_list_cmd():
        ...
        from looprail.config.update_providers import list_providers
    ...
    def provider_get_cmd(
        ...
        from looprail.config.update_providers import get_provider_config
    ...
    def provider_set_cmd(
        ...
        Examples:
            ...
            looprail provider set openrouter --api-key sk-or-v1-...
            looprail provider set azure-openai --api-key X --api-base https://...
            looprail provider set gemini --api-key K --vertex true
        """
        ...
        console.print(f"  [dim]Run 'looprail provider test {name}' to verify the credentials.[/dim]")
    ...
    def provider_test_cmd(
        ...
        hints = {
            "not_configured": f"Run: looprail provider set {name} --api-key <KEY>",
            "invalid_key": f"Run: looprail provider set {name} --api-key <NEW-KEY>",
            "no_credits": "Fund your account at the provider's billing page",
            "rate_limited": "Wait a few minutes and retry, or switch provider",
            "oauth_token_missing": (f"Run: looprail provider login {name.replace('_', '-')}"),
