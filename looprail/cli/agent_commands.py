...
- The interactive ``looprail run`` REPL command body (multiline paste,
...
from looprail.cli._run_surface import CliRunReporter, RunVerbosity, bounded_text, parse_verbosity
...
def _init_prompt_session() -> None:
    ...
    from looprail.config.paths import get_cli_history_path
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} Looprail[/cyan]")
    console.print(body)
    console.print()
...
def register(app: typer.Typer) -> None:
    ...
    def agent(
        ...
        logs: bool = typer.Option(False, "--logs/--no-logs", help="Show Looprail runtime logs during chat"),
        ...
        from looprail.cli.onboard_commands import _is_config_populated

        if message is None and _stdout_isatty() and not _is_config_populated():
            from looprail.cli.onboard_commands import ensure_configured_or_onboard
        ...
        from looprail.tracing import config as tracing_config

        # load_runtime_config 必须最先运行：它调用 set_config_path()，使之后的 load_looprail_config()
        # 从 --config 而非默认 ~/.looprail/config.json 读取。否则 --config 中的 skill_forge 会被静默忽略。
        ...
        # remains under LOOPRAIL_TRACING_DIR or the product trace directory.
        ...
        if resume is not None:
            from looprail.cli.session_commands import resolve_session
            ...
            from looprail.cli.session_commands import resolve_session_cross_channel
        ...
        def _thinking_ctx():
            ...
            return console.status("[dim]Looprail is thinking...[/dim]", spinner="dots")
        ...
        if message:
            ...
            # 段错误。该风险存在时，looprail.cli.commands.run 的退出汇合点会硬退出、跳过收尾，
            # 因此此路径只需正常返回。
        else:
            # 交互模式——用户轮次通过 spine（submit -> lane -> hub -> CliOutlet）运行；
            # Cron 轮次使用同一 spine 和 hub。
            from looprail.cli._repl_spine import build_repl, run_repl_loop
