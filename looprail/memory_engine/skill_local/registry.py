...
``looprail > nanobot > openclaw``。当前优先级实际为 Workspace > 按配置顺序的 External/Extra Dirs > Builtin，
...
``looprail/memory_engine/skills/``；Builtin Markdown 与 Code 同在 ``memory_engine`` Package。
...
#
# 解析为 ``looprail/memory_engine/skills/``；内置 Markdown 库与 Skill 代码一起位于 memory_engine 包内。
...
def _parse_nested_metadata(raw: str) -> dict:
    """从 ``metadata`` JSON Blob 提取 Looprail-compatible Namespaced Metadata。

    Lookup Order First Match Wins：``looprail`` > ``nanobot`` > ``openclaw``。Empty、Invalid JSON、Non-dict 或
    ...
    if not isinstance(data, dict):
        return {}
    for key in ("looprail", "nanobot", "openclaw"):
