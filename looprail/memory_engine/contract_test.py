...
Base Class **位于 Package 内而非 ``tests/``**，原因有二：Plugin Authors 安装 ``looprail`` 后需要从
``looprail`` Import；Pytest 不会从 Non-test Package 自动 Collect，因此 Abstract Base 不会自行运行，只有
...
from looprail.memory_engine.backend import Memory, MemoryBackend
