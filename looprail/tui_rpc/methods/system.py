...
``result: dict``。输入 validation 在可用时使用 ``looprail/tui_rpc/models.py`` 的 Pydantic v2
...
# looprail_version：从已安装元数据读取的 Looprail 发行版本。
...
def _looprail_version() -> str:
    try:
        return _md.version("looprail")
...
async def system_hello(params: dict) -> dict:
    ...
    # pid 用于区分共享同一日志文件的并发 `looprail` 进程。
    logger.info(
...
async def system_version(params: dict) -> dict:
    ...
    ``info.version``，``looprail_version`` 来自 installed package metadata；editable/源码环境
    缺少 metadata 时后者为 ``0.0.0+unknown``。返回值不执行 upgrade 或兼容性决策。
    """
    return {
        "server_version": SERVER_VERSION,
        "schema_version": SCHEMA_VERSION,
        "looprail_version": _looprail_version(),
    }
