...
``~/.looprail/config.json``，例如 ``tui.theme`` 对应
``{"tui": {"theme": "..."}}``，使 legacy ``looprail.config.loader`` 无需额外 schema
转换就能读取同一文件。
...
from looprail.providers.registry import find_by_model
...
from looprail.tui_rpc.methods.turn import has_active_turns
