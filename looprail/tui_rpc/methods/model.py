...
所有写 helper 都位于 ``looprail.config.update_providers``，它是 provider config 的 single write
path；handler 使用 ``asyncio.to_thread`` 包装同步 disk IO，避免阻塞 RPC event loop。
OAuth provider 不允许从 picker 写 key，只能运行 ``looprail provider login``，此限制暴露为
``-32012``。
...
from looprail.providers.registry import find_by_model, find_by_name
from looprail.tui_rpc.errors import (
    ConfigValidationError,
    NotSupportedInV01Error,
)
from looprail.tui_rpc.models import (
    ModelAddModelParams,
...
def _build_provider_entry(
    ...
    *,
    ...
    if is_oauth and not configured:
        warning = f"run `looprail provider login {slug.replace('_', '-')}` to authenticate"
...
async def model_save_key(params: dict) -> dict:
    ...
    ``looprail provider login``；``custom`` 与 ``azure_openai`` 还必须提供 ``api_base``。
    ...
    if spec.is_oauth:
        raise NotSupportedInV01Error(
            f"{spec.label} uses OAuth; run `looprail provider login {parsed.slug.replace('_', '-')}`",
            data={"slug": parsed.slug},
        )
