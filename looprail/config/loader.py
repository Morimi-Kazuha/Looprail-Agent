"""Looprail Base Runtime Configuration 的 Read、Validate、Migrate 与 Write Utilities。

本模块拥有当前 Config Path、Base `Config` Loader、Raw Read-modify-write 安全读取和旧格式迁移。
`EXTENSION_KEYS` 是 Looprail Feature Blocks 的 Single Source of Truth：Base Validation 前可移除，
`load_looprail_config` 则保留并单独解析。
...
# Looprail 扩展配置块键名的唯一事实来源。
# _migrate_config（在基础 Config 校验前 pop）和 load_looprail_config
...
EXTENSION_KEYS = (
    ...
    "runtime",
    # 仓库内可观测性 tracing（looprail.tracing）。
    "tracing",
)
...
def _reject_unsupported_config(data: dict[str, Any]) -> None:
    if "sentinel" in data:
        raise ValueError(
            "Config contains sentinel, which is no longer supported; remove that section before starting Looprail."
        )
    gateway = data.get("gateway")
    if isinstance(gateway, dict) and "heartbeat" in gateway:
        raise ValueError(
            "Config contains gateway.heartbeat, which is no longer supported; remove that section before starting Looprail."
        )
    ...
    if removed:
        name = removed[0]
        raise ValueError(
            f"Config contains channels.{name}, which is no longer supported; remove that section before starting Looprail."
        )
    tools = data.get("tools")
    if isinstance(tools, dict) and "media" in tools:
        raise ValueError(
            "Config contains tools.media, which is no longer supported; remove that section before starting Looprail."
        )
    if isinstance(tools, dict) and any(key in tools for key in ("deepResearch", "deep_research")):
        raise ValueError(
            ...
            "remove that section before starting Looprail."
        )
    for skill_forge_key in ("skillForge", "skill_forge"):
        ...
        if isinstance(weights, dict) and "hub" in weights:
            raise ValueError(
                ...
                "remove that entry before starting Looprail."
            )
