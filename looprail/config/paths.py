...
from looprail.product import DEFAULT_WORKSPACE_SPEC, get_default_workspace, get_product_home, get_project_state_dir
from looprail.utils.helpers import ensure_dir
...
def get_data_dir() -> Path:
    ...
    路径是 Active Config File 的 Parent，因此 ``--config`` 可隔离多个 Looprail Instance State。
    """
...
def get_sandbox_dir(backend: str) -> Path:
    ...
    Layers 位于 Looprail Data Dir 而非 ``~/.boxlite``。函数只分配路径，不创建 VM。
    """
...
def get_cli_history_path() -> Path:
    """返回 Shared CLI History File Path ``<product_home>/.looprail_history``。

    函数不创建文件；多个 CLI Session 共用该路径。
    """
    return get_product_home() / ".looprail_history"
