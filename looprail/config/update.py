"""对 ``~/.looprail/config.json`` 执行 Minimal In-place Updates。

``save_config`` 会 Re-serialize Entire Pydantic Model，把所有 Runtime Defaults Bake 回文件；这里改为读取
Raw JSON，只 Patch 少量 Fields，再用 Temp-file + Rename Atomic Rewrite。``looprail cron config set`` 与
...
from looprail.config.schema import CronConfig
...
def set_default_model(
    ...
    *,
    ...
    Onboarding 在 User 选 Provider 后，把 Default Model 换成 Matching Vendor；否则 ``looprail run`` 仍使用 Fresh
...
def init_extension_block_defaults(*, config_path: Path | None = None) -> None:
    ...
    Onboarding Bootstrap 调用一次，使新 ``~/.looprail/config.json`` 展示可编辑 Knobs。每 Field 只在 Absent 时
