"""Looprail Base Agent Runtime 的 Pydantic Configuration Schema。
...
from looprail.product import DEFAULT_WORKSPACE_SPEC, get_default_workspace
...
class ProvidersConfig(Base):
    ...
    `looprail.providers.registry` 完成；声明 Block 不自动 Import/Connect Provider。
    """
...
class Config(BaseSettings):
    """Looprail Base Agent Runtime 的 Root Configuration。

    组合 Agents、Channels、Providers、Gateway、Tools、Routing、Cron 与 UI Language，并支持 ``LOOPRAIL_`` Env
    Prefix + ``__`` Nested Delimiter。Looprail Feature Extensions 通过 `load_looprail_config` 另行 Compose。
    """
    ...
    def skill_forge(self):
        ...
        Extension Blocks 由 ``load_looprail_config`` 加载，不通过 Base `Config`。该 Property 只为访问 Plain
