"""Looprail Feature Configuration，在 Base `Config` 上组合独立 Feature Blocks。

Usage：

    from looprail.config import LooprailConfig, load_looprail_config

    cfg = load_looprail_config()
    if cfg.context.engine == "curator":
        ...

Design 上，``LooprailConfig`` Compose Base ``Config`` 而非 Subclass，使 Base Schema 不变，Feature Blocks 可
...
from looprail.config.loader import (
    EXTENSION_KEYS,
    _migrate_config,
    get_config_path,
)
from looprail.config.loader import load_config as load_base_config
from looprail.config.schema import Config as BaseConfig
...
class PluginsConfig(_Base):
    ...
    ``disabled`` 是按 Plugin ID（匹配 ``looprail-plugin.toml`` 的 ``id``）索引的 User Opt-out List。``config``
...
class CheckpointConfig(_Base):
    ...
    shadow_dir: str = ".looprail/shadow.git"
...
class TracingConfig(_Base):
    """In-tree ``looprail.tracing`` Observability Config。

    Default On；每个 ``looprail`` Command 在 AgentLoop 构造前安装 Non-invasive Instrumentation。
    ``LOOPRAIL_TRACING=0`` 是覆盖此 Block 的 Explicit Env Kill-switch。``looprail tracing`` 或 TUI ``/tracing``
...
class LooprailConfig(_Base):
    """Looprail Root Config，把 Base `Config` 与 Feature Extensions Compose 在一起。
...
def load_looprail_config(config_path: Path | None = None) -> LooprailConfig:
    """从同一 JSON File 加载 Base Config 与 Looprail Extension Blocks。
    ...
    return LooprailConfig(base=base, **overrides)
