...
from looprail.config.schema import ProvidersConfig
...
from looprail.providers.registry import ProviderSpec, find_by_name
...
def _provider_spec(name: str) -> ProviderSpec:
    ...
    if spec is None:
        raise KeyError(f"No registry entry for provider '{name}'. Add a ProviderSpec to looprail/providers/registry.py.")
    return spec
    ...
    if spec.is_oauth:
        ...
        if forbidden:
            raise RuntimeError(
                ...
                f"{forbidden} directly. Run: looprail provider login "
                f"{name.replace('_', '-')}"
            )
