...
缓存是 ``~/.looprail/cache/`` 下一个带 Version 的 JSON 文件，通过 Temp-file + ``os.replace`` 原子写入，
...
from looprail.config.paths import get_cache_dir

# schema 变更后递增此值，以强制失效所有磁盘缓存文件。
CACHE_VERSION = 1
CACHE_FILENAME = "model-catalog.json"

# 测试接缝：设置后覆盖磁盘路径，确保测试不会触碰真实的 ~/.looprail/cache/。
