...
``looprail.agent`` Submodule 导入不会急切加载 AgentLoop 与 LiteLLM，降低 CLI Cold Start。`__dir__`
...
# 通过 PEP 562 延迟重新导出：导入 ``looprail.agent`` 子模块时不应急切构建 ``AgentLoop``，
# 否则会进一步导入 litellm，成为 CLI 冷启动的主要开销。
_LAZY_EXPORTS = {
    "ContextBuilder": "looprail.agent.context",
    "AgentLoop": "looprail.agent.loop",
    "MemoryStore": "looprail.memory_engine.consolidate.consolidator",
}
