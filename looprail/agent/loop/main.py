...
from looprail.agent.tools.ask_user import AskUserTool
from looprail.agent.tools.base import ToolResult
from looprail.agent.tools.execution import ToolExecution, ToolExecutionContext, ToolInvocation
...
from looprail.call_efficiency.pricing import resolve_context_window
from looprail.context_engine.budget import resolve_context_limits
from looprail.context_engine.history_trimmer import RuntimeContextTrimmer
...
from looprail.sandbox import SandboxConfig, SandboxExecutor, SandboxInitError, build_executor
from looprail.session.manager import Session, SessionManager
from looprail.spine.message import Media
from looprail.spine.turn import Origin
from looprail.tracing import semconv, trace
from looprail.utils.helpers import estimate_prompt_tokens
from looprail.utils.persisted_payload import sanitize_persisted_payload

# 刻意在 ``__init__`` 和 ``_assemble_context_messages`` 内延迟导入 ``looprail.context_engine``，以打破运行时
# 循环导入：``looprail.agent.__init__`` 急切加载 AgentLoop，而 ``looprail.context_engine.curator``
# 又从 ``looprail.agent.context`` 导入 ``ContextBuilder``。如果在此模块顶层导入，会重新进入
...
if TYPE_CHECKING:
    ...
    from looprail.config.schema import ChannelsConfig, ExecToolConfig
    from looprail.context_engine import ContextEngine
    from looprail.context_engine.factory import ContextEngineFactory
    ...
    from looprail.spine.runner import Drain, Emit, TurnOutcome
    from looprail.spine.turn import TurnRequest
...
class AgentLoop:
    ...
    def __init__(
        ...
        #
        # 延迟导入 ``looprail.context_engine`` 的原因见模块顶部关于 ``looprail.agent.__init__`` 循环导入的说明。
        if context_config is None:
            from looprail.config.looprail import ContextConfig

            context_config = ContextConfig()
        if context_engine_factory is None:
            from looprail.context_engine import build_context_engine
        ...
        if runtime_config is None:
            from looprail.config.looprail import RuntimeConfig
    ...
    def _register_default_tools(self) -> None:
        ...
        函数内延迟导入是为避开 `looprail.agent.__init__` 的循环导入边界。
        """
        ...
        if self.cron_service:
            # 延迟导入：CronTool 所在模块会导入 looprail.agent.tools.base，触发 looprail.agent.__init__，
    ...
    async def _assemble_context_messages(
        ...
        *,
        ...
        导入边界。
        """
        from looprail.context_engine import TurnContext  # 延迟导入，原因见模块说明
    ...
    async def _dispatch_backend_store(
        ...
        ``memory.maybe_consolidate`` 执行 Looprail compaction，本方法负责插件 Backend。传入的
    ...
    async def _start_debug_server(self) -> None:
        ...
        主请求仍可运行，同时避免稍后 `looprail sandbox` 只看到“套接字不存在”却没有原因。
        """
        ...
        if cfg.backend == "none":
            ...
            return
        try:
            from looprail.config.paths import get_data_dir
            from looprail.sandbox.debug_server import SandboxDebugServer
            ...
            # 用户已明确选择调试模式；此处静默失败会让用户在稍后看到 `looprail sandbox`
    ...
    async def _run_agent_loop(
        ...
        # 持久化和返回前移除临时的空响应恢复脚手架。Looprail 会用
    ...
    async def _process_message(
        ...
        """
        from looprail.agent.hook import AgentHookContext
    ...
    async def run_turn(
        ...
        *,
        ...
        """
        from looprail.proactive_engine.schedulers.cron.tool import CronTool
        from looprail.spine.events import (
            MediaOut,
            Notice,
            NoticeKind,
            Reasoning,
        ...
        from looprail.spine.message import Media
        from looprail.spine.runner import TurnOutcome
