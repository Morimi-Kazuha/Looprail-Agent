...
from looprail.product import WORKSPACE_STATE_DIRNAME
...
_GIT_IDENT = (
    ...
    "user.name=Looprail",
    "-c",
    "user.email=checkpoint@looprail.local",
    "-c",
...
_DEFAULT_EXCLUDES = """\
# Looprail shadow-git default excludes (see checkpoint.py).
        ...
        try:
            if not (self._git_dir / "HEAD").exists():
                ...
                if rc != 0:
                    ...
                    return False
                # 在影子 Git 旁写入可发现性提示，让注意到 ``.looprail/`` 的用户无需搜索代码库即可识别它。
                # 该操作尽力而为，写入失败不影响主流程。
                try:
                    notice = self._git_dir.parent / "NOTICE.txt"
                    notice.write_text(
                        "This directory is created by Looprail's runtime "
                        ...
                        "in your Looprail config (typically "
                        "~/.looprail/config.json, or whichever file you "
                        "passed via --config).\n",
                        encoding="utf-8",
                    )
                except OSError:
