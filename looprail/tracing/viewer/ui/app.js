...
const I18N = {
  en: {
    ...
    'shutdown.failed': 'Could not stop the viewer. Try looprail tracing --stop in the terminal.',
    'shutdown.stoppedKicker': 'Viewer stopped',
    'shutdown.stoppedTitle': 'Tracing is no longer using this process.',
    'shutdown.stoppedCopy': 'Your trace files are preserved. You can close this tab or run looprail tracing to start again.',
    'view.api': 'API Calls',
    'view.trace': 'Traces',
  ...
  zh: {
    ...
    'shutdown.failed': '无法关闭 Viewer，请在终端运行 looprail tracing --stop。',
    'shutdown.stoppedKicker': 'VIEWER 已关闭',
    'shutdown.stoppedTitle': 'Tracing Viewer 进程已停止。',
    'shutdown.stoppedCopy': 'Trace 文件仍然保留。你可以关闭此标签页，或运行 looprail tracing 再次启动。',
    'view.api': 'API 调用',
    'view.trace': '会话追踪',
    'sessions.title': '会话列表',
    'field.search': '搜索',
...
function renderModelOutputCard(span, artifacts) {
  ...
  // OpenClaw 方言使用 assistantTexts / lastAssistant；Looprail 方言使用 output/content 加顶层
  ...
  const finalMessage =
    ...
    // 将 Looprail 的扁平 {id,name,arguments} 规范化为 messageBodyText 渲染的
...
function renderSubagentCallCard(span, artifacts) {
  ...
  // Looprail 把子智能体最终返回持久化在跨度本身：`Subagent Result` 产物（{task, result}）加
  // `subagent.result_preview` 属性，而不是像下方 OpenClaw 模型那样存为独立子会话。若不处理，
  // 卡片只检查子会话/spawn 工具路径，在 Looprail 中找不到内容并回退到空结果占位，尽管结果就在跨度上。
  ...
  // 前向链接：Looprail 将子智能体作为独立追踪运行，当前分发节点仅作摘要；提供跳转到完整追踪树的入口。
...
function renderSkillReadCard(trace, span, artifacts) {
  ...
  // Looprail 的 read_file→skill.read 把内容/参数放在重新标型工具跨度自身的 Tool Input/Output
  // 产物和 skill.result_preview 中，而非 OpenClaw 的 Skill Read / Read Content / Read Request
  // 产物。需回退到 Looprail 数据，避免卡片为空。
...
// 顶层输入/输出或属性转储。Looprail 自身富卡片在此注册为自定义渲染器，使分发由数据驱动且无回归，
...
function renderContentLegacy(span, trace, artifacts) {
  ...
  // Looprail 的 read_file→skill.read 是重新标型的工具调用，其 Tool Input/Output 就是技能输入/输出，
...
async function stopViewer() {
  ...
  try {
    const response = await fetch('/api/shutdown', {
      method: 'POST',
      headers: { 'X-Looprail-Viewer-Action': 'shutdown' }
    });
