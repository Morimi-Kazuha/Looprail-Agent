...
const SHUTDOWN_HEADER = 'x-looprail-viewer-action';
...
function detectSpanFailure(span, attrs) {
  ...
  if (span?.name === 'subagent.call') {
  // 只有真实失败状态才算失败。OpenClaw 用 'accepted' 表示成功 spawn，Looprail 使用
  // 'ok' / 'completed' / 'ended'。把所有非 'accepted' 状态标红会误伤成功的 Looprail 子智能体；
  ...
  // OpenClaw 的 skill.read 通过 skill.read.* 字节/SHA 属性表示成功；Looprail 的
  // read_file→skill.read 通过 skill.result_preview、skill.path、tool.output.artifact_bytes
  // 表示。任一属性族都可作为真实读取证据，避免成功的 Looprail 读取仅因缺少 OpenClaw 属性而
...
function buildTraceGroups(sessionSpans) {
  ...
  // session.turn 是主追踪根；subagent.run 是子智能体自身追踪的根。Looprail 子智能体作为独立追踪
