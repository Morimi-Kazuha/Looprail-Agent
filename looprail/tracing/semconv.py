"""Looprail Tracing Standard 的 Semantic Conventions。
...
Shape 读取 Runtime 对象，保持 Framework-agnostic；只要 Documented Shapes 不变，就能承受 Looprail Refactor。
...
def _provider_label(model: str | None, provider_class: str | None) -> str | None:
    ...
    Looprail 通过同一 ``LiteLLMProvider`` Class 访问多个 Gateways，Class Name 会隐藏真实 Backend。LiteLLM 在
...
def _llm_input_payload(
    ...
    Looprail 传入 ONE Flat ``messages`` List，包含 System + Prior Turns + Current。函数拆成 Three
...
def _skill_read_path(name: str, params: Any) -> str | None:
    ...
    这是 Discovery→Injection Follow-through：Looprail Summary Mode 告诉 Agent 用 ``read_file`` 读取 Skill Body，
