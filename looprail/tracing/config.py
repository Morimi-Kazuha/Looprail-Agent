"""Looprail In-tree Tracing 的 Lightweight Configuration Accessors。

模块保持 Light 与 Side-effect-free：Process Startup 时由 CLI ``main()`` Callback 读取，决定是否安装
Instrumentation。Environment Variables 是 Explicit Overrides；否则 Looprail Config ``[tracing]`` Section
...
def _config_section() -> dict:
    """Best-effort 读取 Looprail Config File 中的 ``[tracing]`` Block。

    使用 Looprail 自己的 Config-path Resolver，所以 ``--config`` Override 设置后会被 Honor。Missing File、
...
def enabled() -> bool:
    ...
    ``LOOPRAIL_TRACING`` Env 优先；值为 ``0/false/off/no``（Case-insensitive）时关闭，其他显式值开启。Env
    未设置时读取 ``[tracing].enabled``，Section 缺失默认 `True`。
    """
    env = os.environ.get("LOOPRAIL_TRACING")
    if env is not None:
...
def state_dir() -> Path:
    """返回 Trace State Dir，默认 ``~/.looprail/traces``。

    Spans 写入 ``<dir>/logs/audit-spans.log``。``LOOPRAIL_TRACING_DIR`` 可直接 Override，并执行 Expanduser；
    否则基于受 ``LOOPRAIL_HOME`` 影响的 Product Home。函数只计算 Path，不创建目录。
    """
    override = os.environ.get("LOOPRAIL_TRACING_DIR")
    if override:
...
def preview_len() -> int:
    ...
    Full Payloads 应进入 Artifacts。``LOOPRAIL_TRACING_PREVIEW`` Env 优先，其次 ``previewLen`` Config，默认
    500；结果最小为 0，Invalid Value 回退。Preview Limit 只控制可见摘要，不改变 Artifact 原始内容。
    """
    env = os.environ.get("LOOPRAIL_TRACING_PREVIEW")
    if env is not None:
        try:
