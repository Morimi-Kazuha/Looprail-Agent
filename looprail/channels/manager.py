...
from looprail.product import DISTRIBUTION_NAME
...
def _missing_dep_hint(modname: str) -> str:
    ...
    if sys.platform == "win32":
        return (
            "Re-run the Looprail installer from a trusted source checkout: install.ps1. "
            "Alternatively set LOOPRAIL_WHEEL_URL to a maintainer-supplied wheel."
        )
    return (
        "Re-run the Looprail installer from a trusted source checkout: install.sh. "
        "Alternatively set LOOPRAIL_WHEEL_URL to a maintainer-supplied wheel."
    )
