...
真实实现位于 :mod:`looprail.providers.transcription`。Empty ``api_key`` 传为 `None`，让 Provider 自行回退
...
async def transcribe_audio(file_path: str | Path, api_key: str = "", *, channel: str = "") -> str:
    try:
        from looprail.providers.transcription import GroqTranscriptionProvider
