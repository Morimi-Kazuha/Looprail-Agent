"""把 Feishu/Lark 消息接入 Looprail Runtime，并把 Runtime 回复交付回会话。
...
from looprail.channels.errors import transient_network
from looprail.channels.media import save_media_bytes
from looprail.channels.transcribe import transcribe_audio
from looprail.config.schema import FeishuConfig
from looprail.spine.delivery import TerminalDeliveryError
...
class FeishuChannel(ChannelBase):
    ...
    ``ChannelManager`` 管理实例生命周期，``looprail.spine`` 消费已发布的入站消息。
    ...
    def _on_message_sync(self, data: Any) -> None:
        """把 lark WebSocket（WS）线程收到的事件安全地投递到 Looprail 主事件循环。
