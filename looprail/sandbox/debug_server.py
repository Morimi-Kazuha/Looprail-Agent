...
`owned_ids` 中声明拥有的 Running VM，避免调试端跨进程误操作其他 Looprail 实例。
...
from looprail.sandbox._async_utils import cancel_and_collect as _cancel_and_collect
...
class SandboxDebugServer:
    ...
    async def start(self) -> None:
        ...
        告知 “another Looprail process owns the socket”；若 Connection 因 ECONNREFUSED / No Listener 失败，
        ...
        if self._socket_path.exists():
            if await self._probe_alive():
                raise SandboxDebugServerError(
                    ...
                    "another Looprail process is running with debug enabled. "
    ...
    async def _handle_list(self, writer: asyncio.StreamWriter) -> None:
        ...
        from looprail.sandbox._runtime import get_boxlite_runtime
    ...
    async def _resolve_vm(
        ...
        if vm_ref is None:
            ...
            if not candidates:
                await _send(writer, {"type": "error", "message": "No running VMs. Start looprail run/gateway first."})
                return None
            if len(candidates) > 1:
