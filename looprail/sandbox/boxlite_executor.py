...
from looprail.sandbox.interfaces import ExecResult, SandboxExecutor, SandboxInitError
...
class BoxliteExecutor(SandboxExecutor):
    ...
    Executor 通过 Looprail BoxLite Runtime 使用 Raw ``boxlite.Box`` API，而不是 `SimpleBox`，使 One-shot
    ...
    async def _pull_image(self) -> None:
        ...
        SimpleBox 使用与 Working Box 相同的 Looprail Runtime Home，确保 Image 进入同一 Cache；启动后执行
        ...
        async def _do_pull() -> None:
            # 使用 Looprail 的运行时，让镜像进入工作 Box 读取的同一缓存；
        ...
        try:
            ...
            raise SandboxInitError(
                ...
                "  • Tool install: uv tool install --force 'looprail[channels,sandbox]'\n"
