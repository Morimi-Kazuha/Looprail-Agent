...
from looprail.providers.base import LLMProvider, LLMResponse, StreamDelta, ToolCallRequest
from looprail.providers.litellm_setup import import_litellm
from looprail.providers.registry import find_by_model, find_gateway

litellm = import_litellm()
acompletion = litellm.acompletion

# LiteLLM 的异步日志 worker（LoggingWorker）会把队列绑定到单一 event loop。
# Looprail 每个 Turn 使用新 loop（每次调用 asyncio.run），所以下一个 Turn 会重置
# 队列，所有待处理的 ``Logging.async_*_handler`` 协程都会在未 await 的情况下
# 被丢弃。Python 随后打印 ``coroutine ... was never awaited`` RuntimeWarning，
# 并污染 Ink TUI 渲染。被丢弃的是 LiteLLM 自身的成功/失败日志回调，Looprail 不依赖
# 它们。过滤范围只限于 LiteLLM 的 ``Logging`` handler；宽泛的
# ``coroutine '.*'`` 模式也会掩盖 Looprail 自身真正的未 await 错误。
...
# LiteLLM 而非 Looprail。Looprail 尚未发布稳定的公开 URL，因此只发送产品标题；
# 用户提供的 extra_headers 优先级更高。
_OPENROUTER_ATTRIBUTION: dict[str, str] = {
    "X-Title": "Looprail",
    "X-OpenRouter-Title": "Looprail",
...
class LiteLLMProvider(LLMProvider):
    ...
    def _parse_response(self, response: Any) -> LLMResponse:
        """把 LiteLLM Completion Response 解析为 Looprail Standard LLMResponse。
