...
``looprail.context_engine`` 当前只有一个 shipping implementation：:class:`ContextAssembler`。
AgentLoop 经 :func:`looprail.context_engine.build_context_engine` 构建它，并且始终只持有一个
...
:mod:`looprail.agent.context`；后者的 :class:`ContextBuilder` 是 Engine 会调用的低层构建工具，
不是本轮窗口所有者。``AssembledContext`` 与 ``TokenBudget`` 仍位于
:mod:`looprail.memory_engine.base`，因为它们早于本 ABC；``MemoryEngine`` 与 ``ContextEngine`` 是
...
class ContextEngine(ABC):
    ...
    当前唯一实现 :class:`ContextAssembler <looprail.context_engine.assembler.ContextAssembler>`
