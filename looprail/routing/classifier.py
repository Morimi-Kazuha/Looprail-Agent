...
from looprail.routing.types import ClassificationResult, TaskCategory
...
class PromptClassifier:
    ...
    async def classify(self, prompt: str) -> ClassificationResult:
        ...
        if not tasks:
            logger.warning(
                "Embedding data missing — run `python -m looprail.routing.generate_embeddings`. Falling back to 'sanity'."
            )
