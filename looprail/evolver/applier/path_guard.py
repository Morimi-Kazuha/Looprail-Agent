...
file，例如 ``looprail/agent/loop/main.py``，以及 trailing slash directory subtree，例如
``looprail/evolver/``，后者匹配目录自身及全部 descendant。``MUTABLE_OVERRIDES`` 优先，可从较宽
...
IMMUTABLE_PATTERNS: tuple[str, ...] = (
    ...
    "looprail/eval_engine/adapter/",
    "looprail/eval_engine/judge/",
    ...
    "looprail/agent/context/",
    "looprail/agent/tools/base.py",
    "looprail/agent/tools/registry.py",
    "looprail/agent/personalizer/",
    "looprail/agent/subagent/",
    # Looprail 使用 spine 代替上游智能体 API；运行器是每轮必经的桥梁，属于同一契约层级。
    "looprail/agent/spine_runner.py",
    "looprail/spine/",
    "looprail/providers/",
    "looprail/session/",
    "looprail/memory_engine/",
    "looprail/context_engine/",
    "looprail/sandbox/",
    "looprail/security/",
    # 配置：schema/loader（.py）不可变，值文件（.yaml/.json）可变。
    "looprail/config/__init__.py",
    "looprail/config/looprail.py",
    "looprail/config/loader.py",
    "looprail/config/paths.py",
    "looprail/config/schema.py",
    "looprail/config/update.py",
    "looprail/config/update_channels.py",
    "looprail/config/update_providers.py",
    # ── L4——审计/数据完整性 ──────────────────────────────────────────────
    "looprail/eval_engine/hooks/tool_audit_hook.py",
    # ── L5——测试、依赖和 CI ──────────────────────────────────────────────
    "tests/",
    "pyproject.toml",
    "uv.lock",
