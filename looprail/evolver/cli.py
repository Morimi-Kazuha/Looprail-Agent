"""``looprail evolve``：opt-in Beta self-evolution 命令入口。
...
CLI 只解析参数并委托 :mod:`looprail.evolver.launch.runner`。命令返回 0 只表示对应 command 完成；
...
from looprail.product import CLI_NAME


def build_parser() -> argparse.ArgumentParser:
    """构造 ``looprail evolve`` 的 argparse command tree。
...
def main(argv: list[str] | None = None) -> int:
    ...
    from looprail.evolver.launch import runner
