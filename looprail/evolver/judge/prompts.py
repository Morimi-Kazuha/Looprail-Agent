...
WHERE_DESCRIPTIONS: dict[PatchWhere, str] = {
    PatchWhere.system_prompt_template: "Looprail's system prompt templates (looprail/templates/*.md: SOUL, AGENTS, "
    ...
    "fixes live here.",
    PatchWhere.judge_prompt: "Looprail's internal turn-judge prompt (looprail/eval_engine/prompts/*.py, L-B "
    "layer). Affects memory updates, NOT the benchmark verifier; safe to "
    "evolve.",
    PatchWhere.tool_description: "A tool's description string or safety pattern in looprail/agent/tools/*.py. Use "
    "when the model misuses a tool because the description is vague / misleading.",
    PatchWhere.hook_new: "Add a new lifecycle hook file in looprail/agent/hook/<name>.py. Hooks fire at "
    ...
    PatchWhere.hook_modify: "Tune an existing lifecycle hook under looprail/agent/hook/. Use only when the "
    "target is outside the immutable hook base and evaluation substrate.",
    PatchWhere.skill: "Add / edit / retire a skill in looprail/memory_engine/skills/. Skills are "
    "retrievable procedural recipes — use when a class of tasks needs a "
    "reusable how-to.",
    PatchWhere.memory: "Tune Looprail's Memory integration through the public backend contract. Memory "
    "holds factual knowledge (e.g. 'Django >=4 uses async views').",
    PatchWhere.tool_new: "Add a new Tool subclass in looprail/agent/tools/<name>.py. Rare — only when no "
    "existing tool plus prompt nudge can express the needed operation.",
    PatchWhere.loop_override: "Scoped override to looprail/agent/loop/main.py or subsidiary loop logic (code "
    ...
    PatchWhere.context_override: "Scoped override under looprail/context_engine/ (code class). Patches short-lived "
