...
:class:`LitellmBackend` 包装 ``looprail.providers`` 的 ``LLMProvider``，包括 self-hosted Qwen-397B；
...
class LitellmBackend(JudgeLLMBackend):
    """委托 Looprail ``LLMProvider`` 的 Judge backend。
    ...
    def __init__(
        self,
        provider: Any,  # looprail.providers.base.LLMProvider，延迟确定类型
        *,
...
class OpenRouterBackend(JudgeLLMBackend):
    ...
    不经 litellm 是为了让 OpenRouter API key 与 Looprail main provider 分离，并独立核算 external-LLM
    ...
    # 对空内容和暂时性 HTTP 错误重试。OpenRouterBackend 绕过 Looprail 提供商栈，无法直接获得
    ...
    async def call(
        ...
        *,
        ...
        headers = {
            ...
            # OpenRouter 标题用于标识调用方；Looprail 尚未发布稳定的公开 URL，
            # 因此不发送默认 HTTP-Referer。
            "X-Title": "Looprail Evolver",
        }
