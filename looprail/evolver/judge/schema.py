...
class PatchWhere(str, Enum):
    ...
    judge_prompt = "judge_prompt"  # 路径：looprail/eval_engine/prompts/*.py
    tool_description = "tool_description"  # looprail/agent/tools/*.py 中的描述
    hook_new = "hook_new"  # 新建 looprail/agent/hook/<name>.py
    hook_modify = "hook_modify"  # 修改 looprail/eval_engine/hooks/*.py
    skill = "skill"  # 路径：looprail/memory_engine/skills/*
    memory = "memory"
    tool_new = "tool_new"  # 新建 looprail/agent/tools/<name>.py
...
class ProposedComponent:
    ...
    :class:`looprail.evolver.tree.node.PatchComponent`。Judge 阶段只有 natural-language
