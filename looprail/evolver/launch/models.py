...
- ``{provider: openai_compat, base_url, model, ...}`` -> one OpenAI-compatible
  chat endpoint (:func:`looprail.evolver.orchestrator.providers.openai_compat.make_call_fn`).
- ``{provider: claude_cli, model, ...}`` -> ``claude -p`` subprocess per call.
- ``{provider: looprail, model?, api_base?, api_key_env?}`` -> Looprail's
  ``LitellmProvider`` bridged to a sync call_fn; ``model`` omitted falls back
  to Looprail's ``agents.defaults.model`` - so a config file with no
  ``models:`` section evolves with whatever model Looprail itself is running.
...
_DEFAULT_SPEC = {"provider": "looprail"}


def _looprail_default_model() -> str:
    from looprail.config.loader import load_config

    return load_config().agents.defaults.model


def make_looprail_call_fn(
    ...
    *,
    ...
    provider = LiteLLMProvider(
        api_key=api_key,
        api_base=api_base,
        default_model=model or _looprail_default_model(),
    )
    ...
    def call(messages: list) -> str:
        ...
        if not isinstance(content, str) or not content.strip():
            raise RuntimeError("Looprail provider returned empty content")
        return content
...
def _build_call_fn(spec: dict) -> CallFn:
    ...
    kind = spec.get("provider", "looprail")
    kwargs = {k: v for k, v in spec.items() if k != "provider"}
    if kind == "openai_compat":
        from looprail.evolver.orchestrator.providers.openai_compat import make_call_fn
    ...
    if kind == "claude_cli":
        ...
        from looprail.evolver.orchestrator.providers.claude_cli import make_claude_call_fn
        ...
        if shutil.which(claude_bin) is None:
            raise ValueError(
                ...
                "openai_compat/looprail"
            )
        return make_claude_call_fn(model, **kwargs)
    if kind == "looprail":
        model = kwargs.pop("model", None)
        return make_looprail_call_fn(model, **kwargs)
    raise ValueError(f"unknown model provider {kind!r} (expected openai_compat / claude_cli / looprail)")
...
def describe_models(models_cfg: dict) -> dict:
    ...
    Looprail default model 读取失败时写 ``<looprail default>``。返回值供 provenance，不用于创建 provider。
    """
    out = {}
    for role in ("driver", "design", "verdict"):
        ...
        if (
            out[role].get("provider", "looprail") == "looprail"
            and "model" not in out[role]
            and "inherit" not in out[role]
            and "omitted" not in out[role]
        ):
            try:
                out[role]["model"] = _looprail_default_model()
            except Exception:  # noqa: BLE001 — 描述仅尽力生成
                out[role]["model"] = "<looprail default>"
    return out


__all__ = ["CallFn", "build_call_fn", "build_role_call_fns", "describe_models", "make_looprail_call_fn"]
