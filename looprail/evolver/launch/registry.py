...
registered plugin 通常位于 subject repo，而非 installed Looprail package。loader 会把 subject root
...
def load_bench(name: str, repo_root: Optional[Union[str, Path]] = None) -> Callable:
    ...
    plugin 位于 subject repo 的 ``benchmarks/``，不是 installed Looprail package，因此提供
    ...
    if target is None:
        raise ValueError(
            f"unknown bench {name!r}; registered: {sorted(BENCHES)} (add yours to looprail.evolver.launch.registry.BENCHES)"
        )
