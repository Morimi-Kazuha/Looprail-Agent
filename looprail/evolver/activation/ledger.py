...
``LOOPRAIL_ACTIVATION_WORKSPACE``；未设置时静默 no-op，使 product Runtime 不承担 I/O 成本。
...
WORKSPACE_ENV = "LOOPRAIL_ACTIVATION_WORKSPACE"

_workspace_var: ContextVar[str | None] = ContextVar("looprail_activation_workspace", default=None)
...
def activation_beacon(node_id: str, site: str = "", **detail: object) -> None:
    ...
    workspace 优先来自 ``ContextVar``，再读 ``LOOPRAIL_ACTIVATION_WORKSPACE``；两者都不存在时
