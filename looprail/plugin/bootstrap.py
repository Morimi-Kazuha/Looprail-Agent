...
from looprail.plugin.discover import PluginDiscovery
from looprail.plugin.registry import PluginRegistry
...
def assemble_plugin_registry(
    *,
    ...
    entry_points_group: str | None = "looprail.plugins",
    disabled: frozenset[str] = frozenset(),
) -> PluginRegistry:
    """Discover 所有 Manifests，Admit Enabled Items，并返回 Registry。

    ``entry_points_group`` 默认 ``"looprail.plugins"``，这是 Third-party Plugins 在 ``pyproject.toml`` 中
