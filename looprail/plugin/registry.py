...
from looprail.plugin.discover import DiscoveredPlugin, Source
...
# ``looprail.agent.tools.base.Tool``。此处标为 Any，使插件
...
class PluginRegistry:
    ...
    def build_memory_backend(
        ...
        *,
        ...
        """
        from looprail.plugin.context import PluginContext  # 局部导入以规避循环依赖

        factory = self.get_memory_backend_factory(name)
        ctx = PluginContext(
            config=config,
            services=services,
            logger=logger or logging.getLogger(f"looprail.plugin.{name}"),
        )
        return factory(ctx)
...
# 在上下文就绪前调用 __init__）。
from looprail.plugin.context import ServiceLocator  # noqa: E402
