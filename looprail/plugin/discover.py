...
Manifest Admission 仍会验证 Distribution Identity 与 Looprail Version Compatibility；Stable Discovery List
...
from looprail import __version__ as _installed_looprail_version
from looprail.plugin.manifest import PluginManifest

logger = logging.getLogger(__name__)


_MANIFEST_FILENAME = "looprail-plugin.toml"


class PluginCompatibilityError(RuntimeError):
    """Plugin Manifest 排除了 Installed Looprail Version 时抛出。
...
class PluginDiscovery:
    ...
    实例还持有待验证的 Looprail Version，所有 Scan 共享同一兼容标准。
    """
    ...
    def __init__(
        ...
        *,
        ...
        looprail_version: str = _installed_looprail_version,
    ) -> None:
        self._bundled_dir = bundled_dir
        self._user_dir = user_dir
        self._project_dir = project_dir
        self._entry_points_group = entry_points_group
        self._looprail_version = looprail_version
    ...
    def _scan_dir(
        ...
        """在 ``<root>/<plugin_id>/looprail-plugin.toml`` 查找 File-based Plugins。

        Subdir Name 只是 Informational，Canonical Plugin ID 来自 Manifest。两者 Mismatch 会记录日志但仍
        加载；解析失败则 Warning + Skip。每个成功 Manifest 都立即验证 Looprail Compatibility，返回的
    ...
    def _scan_entry_points(self, group: str) -> list[DiscoveredPlugin]:
        ...
        Entry-point Value 指出拥有 ``looprail-plugin.toml`` 的 Package。Discovery 通过 Owning Distribution 的
        Installed File Inventory 定位 Manifest，**不 Import Plugin Package**。每项还验证 Distribution
        Name/Version 与 Manifest Identity、以及 Looprail Compatibility；Identity/Compatibility Error 向上
    ...
    def _validate_compatibility(self, manifest: PluginManifest) -> None:
        if not manifest.looprail:
            return
        try:
            compatible = Version(self._looprail_version) in SpecifierSet(manifest.looprail)
        except (InvalidSpecifier, InvalidVersion) as exc:
            raise PluginCompatibilityError(
                f"plugin {manifest.id!r} has invalid Looprail compatibility metadata; reinstall a compatible plugin"
            ) from exc
        if not compatible:
            raise PluginCompatibilityError(
                f"plugin {manifest.id!r} {manifest.version} requires Looprail {manifest.looprail}, but installed Looprail is "
                f"{self._looprail_version}; install a compatible plugin version or set memory.backend to null"
            )
