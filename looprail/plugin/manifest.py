...
Manifest 是与 Plugin Python Package 一起发布的 TOML File ``looprail-plugin.toml``。它声明 Plugin Identity、
...
class ToolContribution(_ManifestBase):
    ...
    在 Boot 时注册进 Agent Tool Set 的 :class:`~looprail.agent.tools.base.Tool`。每条 Entry 对应 One Tool；
...
class PluginManifest(_ManifestBase):
    """解析后的 ``looprail-plugin.toml`` Pure-data Model。

    通常通过 :meth:`from_toml_path` / :meth:`from_toml_str` 构造，Programmatic Tests 也可直接使用 Raw
    ``__init__``。Frozen Pydantic Model 保存 Identity、Looprail Compatibility、Default Enablement、
