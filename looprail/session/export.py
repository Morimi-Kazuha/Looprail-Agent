...
from looprail.utils.helpers import ensure_dir, safe_filename
...
_EXPORT_SCHEMA = "looprail.session.export.v1"
...
def default_export_path(workspace: Path, key: str) -> Path:
    ...
    Path 是 ``<workspace>/exports/<safe key>.looprail-session.json``；Key 经 safe_filename，函数不创建
    Directory、不检查冲突，也不写 File。Caller 可传 Custom Dest 覆盖。
    """

    return Path(workspace) / "exports" / f"{safe_filename(key)}.looprail-session.json"
