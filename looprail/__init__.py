"""Looprail 是紧凑、可组合、受 Benchmark Gate 约束的 Agent Harness，并提供 Opt-in Evolution Path。

Agent Harness 负责把一次模型调用扩展成可持续工作的 Agent Runtime：组织 Context、执行 Provider 与
Tools、记录调用证据，并在明确门禁下复用或演化能力。Looprail 的 Three Feature Pillars 是：
...
Public 与 Internal Python Namespace 都是 ``looprail``。License 与 Third-party Attribution Details 见
...
try:
    __version__ = _pkg_version("looprail")
