...
本模块把 Hermes Logic 包装为 `TokenStrategy`，使它能与 Looprail 自己的 `CacheOptimizer` 一起安装进
`StrategyRegistry`，进行 A/B Comparison。

相对 Looprail `CacheOptimizer` 的 Key Behavioral Differences：它 **不** 标记 Tools Schema，四个断点
