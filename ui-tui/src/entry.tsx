...
// 此处删除原 shebang。我们从 Python 通过子进程运行 entry.js（见 looprail/cli/tui_commands.py），
...
// LOOPRAIL_TUI_COLOR / NO_COLOR 覆盖。
...
// `looprail --print-colors` 是无 IPC 诊断：以色块输出解析后的调色板并退出。在 TTY 防护前运行，
// 因此管道中也能工作，并遵守所有 --color / LOOPRAIL_TUI_COLOR 覆盖。
if (process.env.LOOPRAIL_TUI_PRINT_COLORS === '1') {
  process.stdout.write(renderColorSwatches(DEFAULT_THEME, activeColorTier()))
  process.exit(0)
}

// `looprail --preview-colors` 在真实 UI 上下文（横幅、状态栏、补全行、差异行）中渲染令牌，
// 便于直观检查前景/背景配对和横幅渐变，而不只是孤立色块。
if (process.env.LOOPRAIL_TUI_COLOR_PREVIEW === '1') {
  process.stdout.write(renderColorPreview(DEFAULT_THEME, activeColorTier()))
  process.exit(0)
}

if (!process.stdin.isTTY) {
  console.log('looprail-tui: no TTY')
  process.exit(0)
}
...
// `looprail --check` 是无 IPC 冒烟路径；我们要验证的是导入链和终端重置成功。套接字传输使
// `gw.start()`（约第 53 行）通过 Unix 套接字发送真实 `system.hello` RPC，因此文件底部旧
// LOOPRAIL_TUI_CHECK 处理器对检查路径不可达：无套接字时 gw.start() 先拒绝，而下方
// LOOPRAIL_RPC_SOCKET 防护原本会以状态 2 退出，并对实际上由父进程在 --check 模式启动的子进程
// 给出误导性“通过父进程启动”消息。此处短路，使 --check 保持纯导入冒烟。
if (process.env.LOOPRAIL_TUI_CHECK === '1') {
  process.exit(0)
}

const socketPath = process.env.LOOPRAIL_RPC_SOCKET

if (!socketPath) {
  process.stderr.write('looprail-tui: LOOPRAIL_RPC_SOCKET env var required; spawn via `looprail` parent\n')
  process.exit(2)
}
...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `looprail-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? '(failed)'}\n`
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    process.stderr.write(`looprail-tui ${scope}: ${message.slice(0, 2000)}\n`)
  },
  onSignal: signal => {
    resetTerminalModes()
    process.stderr.write(`looprail-tui: received ${signal}\n`)
  }
})
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    ...
    process.stderr.write('looprail-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
  onHigh: (snap, dump) => process.stderr.write(dumpNotice(snap, dump))
})

if (process.env.LOOPRAIL_HEAPDUMP_ON_START === '1') {
