...
`looprail` with no source checkout. But ``dist/`` is a build artifact and is
...
class CustomBuildHook(BuildHookInterface):
    def initialize(self, version: str, build_data: dict) -> None:
        ...
        if dist.is_dir() and (dist / "entry.js").is_file():
            # Map source -> path inside the wheel's `looprail` package.
            build_data.setdefault("force_include", {})[str(dist)] = "looprail/ui-tui/dist"
        else:
            self.app.display_warning(
                "ui-tui/dist/entry.js not found — building WITHOUT the bundled TUI. "
                "`looprail` from this wheel will not work; run "
